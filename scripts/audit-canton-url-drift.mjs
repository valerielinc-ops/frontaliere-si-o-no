...
export function buildAlertBody(record, verdict, trend) {
  ...
  return [
    ...
    buildScheda({
      causa: [
        '(ipotesi, da confermare prima di toccare codice.) La precedenza in',
      ...
      fix: [
        "Dipende da quale delle tre sorgenti sopra regge all'esame; non",
      ...
      metrica: `prima=${(record.rate * 100).toFixed(2)}% atteso=<${(verdict.threshold * 100).toFixed(2)}%`,
      comando: repro,
      note: [
      ...
      fallimento: '`Canton URL drift: il tasso settimanale non scende sotto la soglia`',
    }),
    '## FILE',
    '',
