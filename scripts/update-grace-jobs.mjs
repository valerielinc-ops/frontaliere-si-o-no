...
function inferCategory(title = '', description = '') {
  ...
  if (/\b(trainee|apprentice|intern(?:ship)?s?(?=\W|$)|stages?(?=\W|$))/i.test(hay)) return 'formazione';
...
function inferEmploymentType(text = '') {
  ...
  if (t.includes('trainee') || /\bintern(?:ship)?s?(?=\W|$)/.test(t) || t.includes('apprentice') || /\bstages?(?=\W|$)/.test(t)) return 'internship';
  return 'full_time';
}
