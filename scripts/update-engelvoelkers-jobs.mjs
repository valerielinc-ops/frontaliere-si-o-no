...
function inferCategory(title = '', department = '') {
  ...
  if (/\b(stages?(?=\W|$)|intern(?:ship)?s?(?=\W|$)|tirocinante|junior|trainee|apprentice)/i.test(haystack)) return 'internship';
...
function inferEmploymentType(rawType = '') {
  ...
  if (/\b(stages?(?=\W|$)|intern(?:ship)?s?(?=\W|$))/i.test(t)) return 'internship';
