...
function inferCategory(title = '', jobFunction = '') {
  ...
  if (/\b(intern(?:ship)?s?(?=\W|$)|stages?(?=\W|$)|werkstudent|trainee)/i.test(haystack)) return 'internship';
...
function inferSector(jobFunction = '') {
  ...
  if (/\bintern(?:ship)?s?(?=\W|$)/.test(fn)) return 'Stage & Formazione';
...
function mapEmploymentType(jobType = '', contractType = '') {
  ...
  if (/\bintern(?:ship)?s?(?=\W|$)/.test(ct) || ct.includes('trainee')) return 'internship';
