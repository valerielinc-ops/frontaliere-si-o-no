...
function urlToHub(url) {
  ...
  if (segments.length !== cursor + 2 || !JOB_SECTION_SLUGS.has(section) || !citySegment) return null;
  const dashIdx = citySegment.indexOf('-');
  if (dashIdx <= 0) return null;
  const prefix = citySegment.slice(0, dashIdx);
  const citySlug = citySegment.slice(dashIdx + 1);
  if (!LOCATION_PREFIXES.has(prefix) || !citySlug) return null;
  return { locale, citySlug, url };
}
