#!/usr/bin/env node
/**
 * Blatter's Arosa Hotel job parser — Fetcher and job builder.
 *
 * Source: https://www.hotelcareer.ch/jobs/blatter-s-hotel-arosa-4340?intcid=autosuggest-company-4340
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllBlattersHotelJobs()  — Fetch and parse all jobs
 *   - isBlattersHotelJob()         — Match jobs belonging to this company
...
function isBlattersHotelListingUrl(rawUrl = '') {
  try {
    const url = new URL(rawUrl);
    const host = url.hostname.toLowerCase();
    const path = url.pathname.toLowerCase();
    return (host === BLATTERS_HOTEL_COMPANY_DOMAIN || host.endsWith(`.${BLATTERS_HOTEL_COMPANY_DOMAIN}`))
      && (path === BLATTERS_HOTEL_PATH || path.startsWith(`${BLATTERS_HOTEL_PATH}/`));
  } catch {
    return false;
  }
}
...
/**
 * Check if a job belongs to Blatter's Arosa Hotel.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isBlattersHotelJob(job) {
  ...
  return (
    ...
    isBlattersHotelListingUrl(url)
  );
}

/**
 * Validate that a URL belongs to Blatter's Arosa Hotel's domain.
 */
...
/**
 * Fetch all Blatter's Arosa Hotel jobs.
...
export async function fetchAllBlattersHotelJobs() {
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} blatters-hotel ch`);
    const urlHash = createHash('sha1').update(publicUrl).digest('hex').slice(0, 12);

    const job = {
      // ── Required fields ──
      id: `blatters-hotel-${urlHash}`,
      slug: jobSlug,
      slugByLocale: { [sourceLang]: jobSlug },
      company: BLATTERS_HOTEL_COMPANY_NAME,
      companyKey: BLATTERS_HOTEL_KEY,
      companyDomain: BLATTERS_HOTEL_COMPANY_DOMAIN,
      title,
  ...
  console.log(`\n📋 Total Blatter's Arosa Hotel jobs discovered: ${jobs.length}`);
  return jobs;
}
