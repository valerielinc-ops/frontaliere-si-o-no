...
function heuristicTranslateJobTitle(title = '', locale = 'it') {
  let out = normalizeSpace(title || '');
  if (!out) return out;
  const dict = {
    it: [
    ...
    de: [
      ...
      [/\bOperator\b/gi, 'Bediener'],
      [/\bArchitect\b/gi, 'Architekt'],
      [/\bDesigner\b/gi, 'Designer'],
      [/\bHead of\b/gi, 'Leiter'],
      [/\bTeam Lead\b/gi, 'Teamleiter'],
      [/\bProject Manager\b/gi, 'Projektleiter'],
      [/\bMarketing\b/gi, 'Marketing'],
  ...
  for (const [rx, replacement] of (dict[locale] || [])) {
