 ...
 *   - fetchAllAnkerSwissJobs()  — Fetch and parse all jobs
 *   - isAnkerSwissJob()         — Match jobs belonging to this company
...
export const ANKER_SWISS_KEY = 'anker-swiss';
export const ANKER_SWISS_COMPANY_NAME = 'Anker Swiss Ticino AG';
export const ANKER_SWISS_COMPANY_DOMAIN = 'anker-swiss.ch';

const CAREER_URL = 'https://anker-swiss.ch/stellen/';
...
/**
 * Check if a job belongs to Anker Swiss Ticino AG.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isAnkerSwissJob(job) {
  ...
  return (
    key === ANKER_SWISS_KEY ||
    key.startsWith('anker-swiss') ||
    company.includes('anker swiss ticino ag') ||
    url.includes('anker-swiss.ch')
  );
}

/**
 * Validate that a URL belongs to Anker Swiss Ticino AG's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'anker-swiss.ch' || host.endsWith('.anker-swiss.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(ANKER_SWISS_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all Anker Swiss Ticino AG jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllAnkerSwissJobs() {
  console.log(`🔍 Fetching Anker Swiss Ticino AG jobs`);
