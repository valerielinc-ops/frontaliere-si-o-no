 ...
 * tests) — same rationale as HARD_BOUNCE_PATTERN's move into
 * scripts/lib/suppressionDecay.mjs.
 *
 ...
 * treats as "leave suppressed". This is the opposite default from
 * `isRetryable()` in scripts/lib/mailtrapSuppressionRetry.mjs, which treats
 * a missing age signal as immediately retryable ("no age signal at all —
 * don't block forever on missing data"). The two are not actually the same
 * question asked twice: `isRetryable()` only flips a doc to `pending` and
 * lets the normal send cascade + webhook self-heal on the next bounce (a
 * cheap, reversible action), while `classify()` decides between `confirmed`
 * and `pending` for a claim about root cause (was THIS suppression really
 * caused by the mis-mapped `suspension` webhook?) that fabricated consent
 * would make worse than the bug it repairs — so on missing evidence it
