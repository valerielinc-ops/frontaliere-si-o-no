#!/usr/bin/env node
/**
 * benu job parser — Fetcher and job builder.
 *
 * Source: https://job.benu.ch/jobs
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllBenuJobs()  — Fetch and parse all jobs
 *   - isBenuJob()         — Match jobs belonging to this company
...
export const BENU_KEY = 'benu';
export const BENU_COMPANY_NAME = 'benu';
export const BENU_COMPANY_DOMAIN = 'job.benu.ch';

const CAREER_URL = 'https://job.benu.ch/jobs';
...
/**
 * Check if a job belongs to benu.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isBenuJob(job) {
  ...
  return (
    key === BENU_KEY ||
    key.startsWith('benu-') ||
    company.includes('benu') ||
    url.includes('job.benu.ch')
  );
}

/**
 * Validate that a URL belongs to benu's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'job.benu.ch' || host.endsWith('.job.benu.ch');
  } catch {
    return false;
  }
}
 ...
 */
export async function fetchAllBenuJobs() {
  console.log(`🔍 Fetching benu jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} benu ch`);
    ...
    const job = {
      ...
      company: BENU_COMPANY_NAME,
      companyKey: BENU_KEY,
      companyDomain: BENU_COMPANY_DOMAIN,
      title,
  ...
  console.log(`\n📋 Total benu jobs discovered: ${jobs.length}`);
  return jobs;
}
