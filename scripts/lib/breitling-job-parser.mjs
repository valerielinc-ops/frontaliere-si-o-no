...
function detectExperienceLevel(title = '') {
  const t = normalize(title);
  if (/\b(intern(?:ship)?s?(?=\W|$)|apprenti|apprenticeship|stages?(?=\W|$)|stagiaire|trainee)/.test(t)) return 'intern';
...
function detectEmploymentType(title = '') {
  const t = normalize(title);
  if (/\b(intern(?:ship)?s?(?=\W|$)|apprenti|apprenticeship|stages?(?=\W|$)|stagiaire)/.test(t)) return 'INTERN';
