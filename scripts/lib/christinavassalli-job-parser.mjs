#!/usr/bin/env node
/**
 * CHRISTINA VASSALLI Services, Inhaberin Denise Tschäppät job parser — Fetcher and job builder.
 *
 * Source: https://christinavassalli.ch/jobs/
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllChristinavassalliJobs()  — Fetch and parse all jobs
 *   - isChristinavassalliJob()         — Match jobs belonging to this company
...
export const CHRISTINAVASSALLI_KEY = 'christinavassalli';
export const CHRISTINAVASSALLI_COMPANY_NAME = 'CHRISTINA VASSALLI Services, Inhaberin Denise Tschäppät';
export const CHRISTINAVASSALLI_COMPANY_DOMAIN = 'christinavassalli.ch';

const CAREER_URL = 'https://christinavassalli.ch/jobs/';
...
/**
 * Check if a job belongs to CHRISTINA VASSALLI Services, Inhaberin Denise Tschäppät.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isChristinavassalliJob(job) {
  ...
  return (
    key === CHRISTINAVASSALLI_KEY ||
    key.startsWith('christinavassalli-') ||
    company.includes('christina vassalli services, inhaberin denise tschäppät') ||
    url.includes('christinavassalli.ch')
  );
}

/**
 * Validate that a URL belongs to CHRISTINA VASSALLI Services, Inhaberin Denise Tschäppät's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'christinavassalli.ch' || host.endsWith('.christinavassalli.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(CHRISTINAVASSALLI_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all CHRISTINA VASSALLI Services, Inhaberin Denise Tschäppät jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllChristinavassalliJobs() {
  console.log(`🔍 Fetching CHRISTINA VASSALLI Services, Inhaberin Denise Tschäppät jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} christinavassalli ch`);
    const urlHash = createHash('sha1').update(publicUrl).digest('hex').slice(0, 12);

    const job = {
      // ── Required fields ──
      id: `christinavassalli-${urlHash}`,
      slug: jobSlug,
      slugByLocale: { [sourceLang]: jobSlug },
      company: CHRISTINAVASSALLI_COMPANY_NAME,
      companyKey: CHRISTINAVASSALLI_KEY,
      companyDomain: CHRISTINAVASSALLI_COMPANY_DOMAIN,
      title,
