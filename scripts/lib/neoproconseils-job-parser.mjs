#!/usr/bin/env node
/**
 * NEO Pro conseils SA job parser — Fetcher and job builder.
 *
 * Source: https://neoproconseils.ch/emplois/
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllNeoproconseilsJobs()  — Fetch and parse all jobs
 *   - isNeoproconseilsJob()         — Match jobs belonging to this company
...
export const NEOPROCONSEILS_KEY = 'neoproconseils';
export const NEOPROCONSEILS_COMPANY_NAME = 'NEO Pro conseils SA';
export const NEOPROCONSEILS_COMPANY_DOMAIN = 'neoproconseils.ch';

const CAREER_URL = 'https://neoproconseils.ch/emplois/';
...
/**
 * Check if a job belongs to NEO Pro conseils SA.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isNeoproconseilsJob(job) {
  ...
  return (
    key === NEOPROCONSEILS_KEY ||
    key.startsWith('neoproconseils-') ||
    company.includes('neo pro conseils sa') ||
    url.includes('neoproconseils.ch')
  );
}

/**
 * Validate that a URL belongs to NEO Pro conseils SA's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'neoproconseils.ch' || host.endsWith('.neoproconseils.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(NEOPROCONSEILS_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all NEO Pro conseils SA jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllNeoproconseilsJobs() {
  console.log(`🔍 Fetching NEO Pro conseils SA jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} neoproconseils ch`);
    const urlHash = createHash('sha1').update(publicUrl).digest('hex').slice(0, 12);

    const job = {
      // ── Required fields ──
      id: `neoproconseils-${urlHash}`,
      slug: jobSlug,
      slugByLocale: { [sourceLang]: jobSlug },
      company: NEOPROCONSEILS_COMPANY_NAME,
      companyKey: NEOPROCONSEILS_KEY,
      companyDomain: NEOPROCONSEILS_COMPANY_DOMAIN,
      title,
  ...
  console.log(`\n📋 Total NEO Pro conseils SA jobs discovered: ${jobs.length}`);
  return jobs;
}
