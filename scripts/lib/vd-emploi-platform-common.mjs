...
function detectCategory(title = '', jobCat = '') {
  ...
  if (/\b(lernend|praktik|ausbildung|apprenti|stages?(?=\W|$)|stagiaire)/.test(t)) return 'Formazione';
  return 'Sanità / Ospedali';
}

function detectExperienceLevel(title = '') {
  const t = normalize(title);
  if (/\b(praktik|stages?(?=\W|$)|stagiaire|intern(?:ship)?s?(?=\W|$)|lehrling|lernend|apprenti)/.test(t)) return 'intern';
