#!/usr/bin/env node
/**
 * Medizinisches Center Bonaduz job parser — Fetcher and job builder.
 *
 * Source: https://mcm-group.ch/jobs
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllMcmGroupJobs()  — Fetch and parse all jobs
 *   - isMcmGroupJob()         — Match jobs belonging to this company
...
export const MCM_GROUP_KEY = 'mcm-group';
export const MCM_GROUP_COMPANY_NAME = 'Medizinisches Center Bonaduz';
export const MCM_GROUP_COMPANY_DOMAIN = 'mcm-group.ch';

const CAREER_URL = 'https://mcm-group.ch/jobs';
...
/**
 * Check if a job belongs to Medizinisches Center Bonaduz.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isMcmGroupJob(job) {
  ...
  return (
    key === MCM_GROUP_KEY ||
    key.startsWith('mcm-group-') ||
    company.includes('medizinisches center bonaduz') ||
    url.includes('mcm-group.ch')
  );
}

/**
 * Validate that a URL belongs to Medizinisches Center Bonaduz's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'mcm-group.ch' || host.endsWith('.mcm-group.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(MCM_GROUP_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all Medizinisches Center Bonaduz jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllMcmGroupJobs() {
  console.log(`🔍 Fetching Medizinisches Center Bonaduz jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} mcm-group ch`);
    const urlHash = createHash('sha1').update(publicUrl).digest('hex').slice(0, 12);

    const job = {
      // ── Required fields ──
      id: `mcm-group-${urlHash}`,
      slug: jobSlug,
      slugByLocale: { [sourceLang]: jobSlug },
      company: MCM_GROUP_COMPANY_NAME,
      companyKey: MCM_GROUP_KEY,
      companyDomain: MCM_GROUP_COMPANY_DOMAIN,
      title,
      ...
      canton,
      url: publicUrl,
      source: 'Medizinisches Center Bonaduz Dedicated Parser',
      sourceLang,
  ...
  console.log(`\n📋 Total Medizinisches Center Bonaduz jobs discovered: ${jobs.length}`);
  return jobs;
}
