 ...
 *   - fetchAllIpersonalJobs()  — Fetch and parse all jobs
 *   - isIpersonalJob()         — Match jobs belonging to this company
...
export function isTrustedDomain(rawUrl = '') {
  try {
    ...
    return host === 'med-ipersonal.ch' || host.endsWith('.med-ipersonal.ch');
  } catch {
    return false;
  }
}
...
export async function fetchAllIpersonalJobs() {
  ...
  for (const listing of listings) {
    ...
    // letterale: qui la chiave (`ipersonal`) e' il brand del gemello, non il
    // datore di questo host, e cementarla nell'URL fa nascere ogni riga nuova
    // con un datore che il record non dichiara (#7722).
    const jobSlug = slugify(`${title} ${IPERSONAL_COMPANY_NAME} ch`);
    const urlHash = createHash('sha1').update(publicUrl).digest('hex').slice(0, 12);

    const job = {
      // ── Required fields ──
      id: `ipersonal-${urlHash}`,
      slug: jobSlug,
