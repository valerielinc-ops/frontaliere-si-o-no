#!/usr/bin/env node
/**
 * gmo job parser — Fetcher and job builder.
 *
 * Source: https://www.arsante.ch/emploi
 *
...
export const GMO_KEY = 'gmo';
export const GMO_COMPANY_NAME = 'gmo';
export const GMO_COMPANY_DOMAIN = 'arsante.ch';

const CAREER_URL = 'https://www.arsante.ch/emploi';
...
/**
 * Check if a job belongs to gmo.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isGmoJob(job) {
  ...
  return (
    key === GMO_KEY ||
    key.startsWith('gmo') ||
    company.includes('gmo') ||
    url.includes('arsante.ch')
  );
}

/**
 * Validate that a URL belongs to gmo's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'arsante.ch' || host.endsWith('.arsante.ch');
  } catch {
    return false;
  }
}
 ...
 */
export async function fetchAllGmoJobs() {
  console.log(`🔍 Fetching gmo jobs`);
  ...
  for (const listing of listings) {
    ...
    const job = {
      ...
      description: descriptionText || `${title} — gmo`,
      descriptionByLocale: { [sourceLang]: descriptionText || `${title} — gmo` },
      location,
      canton,
      url: publicUrl,
