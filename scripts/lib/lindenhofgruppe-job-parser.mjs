...
function detectExperienceLevel(title = '') {
  const t = normalize(title);
  if (/\b(praktik|stages?(?=\W|$)|intern(?:ship)?s?(?=\W|$)|lehrling|lernend|apprenti)/.test(t)) return 'intern';
...
async function fetchJobListings() {
  ...
  while (offset < total) {
    ...
    const items = assertJsonListShape(data, { key: 'jobs', source: 'lindenhofgruppe' });
