#!/usr/bin/env node
/**
 * Bistro Guggerzyt job parser — Fetcher and job builder.
 *
 * Source: https://www.guggerbach.ch/guggerbach/jobs-karriere
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllGuggerbachJobs()  — Fetch and parse all jobs
 *   - isGuggerbachJob()         — Match jobs belonging to this company
...
export const GUGGERBACH_KEY = 'guggerbach';
export const GUGGERBACH_COMPANY_NAME = 'Bistro Guggerzyt';
export const GUGGERBACH_COMPANY_DOMAIN = 'guggerbach.ch';

const CAREER_URL = 'https://www.guggerbach.ch/guggerbach/jobs-karriere';
...
/**
 * Check if a job belongs to Bistro Guggerzyt.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isGuggerbachJob(job) {
...
/**
 * Validate that a URL belongs to Bistro Guggerzyt's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'guggerbach.ch' || host.endsWith('.guggerbach.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(GUGGERBACH_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all Bistro Guggerzyt jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllGuggerbachJobs() {
  console.log(`🔍 Fetching Bistro Guggerzyt jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} guggerbach ch`);
    const urlHash = createHash('sha1').update(publicUrl).digest('hex').slice(0, 12);

    const job = {
      // ── Required fields ──
      id: `guggerbach-${urlHash}`,
      slug: jobSlug,
      slugByLocale: { [sourceLang]: jobSlug },
      company: GUGGERBACH_COMPANY_NAME,
      companyKey: GUGGERBACH_KEY,
      companyDomain: GUGGERBACH_COMPANY_DOMAIN,
      title,
