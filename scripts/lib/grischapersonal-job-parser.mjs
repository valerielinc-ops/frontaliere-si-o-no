#!/usr/bin/env node
/**
 * Grischa Personal AG job parser — Fetcher and job builder.
 *
 * Source: https://grischapersonal.ch/stellen/
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllGrischapersonalJobs()  — Fetch and parse all jobs
 *   - isGrischapersonalJob()         — Match jobs belonging to this company
...
export const GRISCHAPERSONAL_KEY = 'grischapersonal';
export const GRISCHAPERSONAL_COMPANY_NAME = 'Grischa Personal AG';
export const GRISCHAPERSONAL_COMPANY_DOMAIN = 'grischapersonal.ch';

const CAREER_URL = 'https://grischapersonal.ch/stellen/';
...
/**
 * Check if a job belongs to Grischa Personal AG.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isGrischapersonalJob(job) {
  ...
  return (
    key === GRISCHAPERSONAL_KEY ||
    key.startsWith('grischapersonal-') ||
    company.includes('grischa personal ag') ||
    url.includes('grischapersonal.ch')
  );
}

/**
 * Validate that a URL belongs to Grischa Personal AG's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'grischapersonal.ch' || host.endsWith('.grischapersonal.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(GRISCHAPERSONAL_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all Grischa Personal AG jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllGrischapersonalJobs() {
  console.log(`🔍 Fetching Grischa Personal AG jobs`);
  ...
  for (const listing of listings) {
    ...
    const job = {
      // ── Required fields ──
      id: `grischapersonal-${urlHash}`,
      slug: jobSlug,
      slugByLocale: { [sourceLang]: jobSlug },
      company: GRISCHAPERSONAL_COMPANY_NAME,
      companyKey: GRISCHAPERSONAL_KEY,
      companyDomain: GRISCHAPERSONAL_COMPANY_DOMAIN,
      title,
  ...
  console.log(`\n📋 Total Grischa Personal AG jobs discovered: ${jobs.length}`);
  return jobs;
}
