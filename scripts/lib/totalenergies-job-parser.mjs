...
function detectExperienceLevel(title = '', experienceRaw = '') {
  ...
  if (/\b(intern(?:ship)?s?(?=\W|$)|apprentice|vie\b)/.test(raw)) return 'intern';
  ...
  const t = normalize(title);
  if (/\b(intern(?:ship)?s?(?=\W|$)|internship|vie\b|stagiaire|apprentice)/.test(t)) return 'intern';
...
function detectEmploymentType(contractRaw = '') {
  ...
  if (/\b(intern(?:ship)?s?(?=\W|$)|apprentice)/.test(raw)) return 'INTERN';
