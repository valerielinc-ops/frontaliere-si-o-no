 ...
 *   - fetchAllBitfinexJobs()  — Fetch and parse all jobs
 *   - isBitfinexJob()         — Match jobs belonging to this company
...
export const BITFINEX_KEY = 'bitfinex';
...
/**
 * Check if a job belongs to Bitfinex.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isBitfinexJob(job) {
  ...
  return (
    key === BITFINEX_KEY ||
    key.startsWith('bitfinex') ||
    company.includes('bitfinex') ||
    url.includes('bitfinex.com')
  );
}

/**
 * Validate that a URL belongs to Bitfinex's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'bitfinex.com' || host.endsWith('.bitfinex.com') || host.endsWith('.recruitee.com');
  } catch {
    return false;
  }
}
...
/**
 * Fetch all Bitfinex jobs from the Recruitee API.
...
export async function fetchAllBitfinexJobs() {
  ...
  for (const offer of offers) {
    ...
    // Work model — Bitfinex is remote-first
    ...
    const job = {
      ...
      canton: jobCanton,
      url: detailUrl,
      source: 'bitfinex-careers-crawler',
      sourceLang,
