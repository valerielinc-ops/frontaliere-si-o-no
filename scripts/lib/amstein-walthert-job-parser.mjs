...
function detectCategory(title = '') {
  ...
  if (/\b(werkstudent|apprenti|praktik|stages?(?=\W|$)|lehrling|lernend)/.test(t)) return 'Formazione';
  return 'Ingegneria';
}

function detectExperienceLevel(title = '') {
  const t = normalize(title);
  if (/\b(werkstudent|apprenti|praktik|stages?(?=\W|$)|stagiair|intern(?:ship)?s?(?=\W|$)|apprendist|lehrling|lernend)/.test(t)) return 'intern';
...
function detectEmploymentType(title = '') {
  const t = normalize(title);
  if (/\b(werkstudent|apprenti|praktik|stages?(?=\W|$)|lehrling|lernend)/.test(t)) return 'INTERN';
