 ...
 *   - isApplyJob()         — Match jobs belonging to this company
...
/**
 * Check if a job belongs to LEITpuls AG.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isApplyJob(job) {
...
/**
 * Validate that a URL belongs to LEITpuls AG's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'apply.refline.ch' || host.endsWith('.apply.refline.ch');
  } catch {
    return false;
  }
}
...
/**
 * Fetch all LEITpuls AG jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllApplyJobs() {
  console.log(`🔍 Fetching LEITpuls AG jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} apply ch`);
    ...
    const job = {
      ...
      company: APPLY_COMPANY_NAME,
      companyKey: APPLY_KEY,
      companyDomain: APPLY_COMPANY_DOMAIN,
      title,
      ...
      canton,
      url: publicUrl,
      source: 'LEITpuls AG Dedicated Parser',
      sourceLang,
  ...
  console.log(`\n📋 Total LEITpuls AG jobs discovered: ${jobs.length}`);
  return jobs;
}
