#!/usr/bin/env node
/**
 * endeso GmbH job parser — Fetcher and job builder.
 *
 * Source: https://endeso.ch/stellenangebote/
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllEndesoJobs()  — Fetch and parse all jobs
 *   - isEndesoJob()         — Match jobs belonging to this company
...
export const ENDESO_KEY = 'endeso';
export const ENDESO_COMPANY_NAME = 'endeso GmbH';
export const ENDESO_COMPANY_DOMAIN = 'endeso.ch';

const CAREER_URL = 'https://endeso.ch/stellenangebote/';
...
/**
 * Check if a job belongs to endeso GmbH.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isEndesoJob(job) {
  ...
  return (
    key === ENDESO_KEY ||
    key.startsWith('endeso-') ||
    company.includes('endeso gmbh') ||
    url.includes('endeso.ch')
  );
}

/**
 * Validate that a URL belongs to endeso GmbH's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'endeso.ch' || host.endsWith('.endeso.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(ENDESO_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all endeso GmbH jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllEndesoJobs() {
  console.log(`🔍 Fetching endeso GmbH jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} endeso ch`);
    ...
    const job = {
      ...
      company: ENDESO_COMPANY_NAME,
      companyKey: ENDESO_KEY,
      companyDomain: ENDESO_COMPANY_DOMAIN,
      title,
  ...
  console.log(`\n📋 Total endeso GmbH jobs discovered: ${jobs.length}`);
