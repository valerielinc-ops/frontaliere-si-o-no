...
const CAREER_URL = 'https://cippatrasporti.altamiraweb.com/';
...
async function fetchJobListings() {
  const spec = loadSpec(CIPPATRASPORTI_KEY);
...
export async function fetchAllCippatrasportiJobs() {
  ...
  for (const listing of listings) {
    ...
    const job = {
      ...
      description: descriptionText || `${title} — Cippà Trasporti SA`,
      descriptionByLocale: { [sourceLang]: descriptionText || `${title} — Cippà Trasporti SA` },
      location,
      canton,
      url: publicUrl,
