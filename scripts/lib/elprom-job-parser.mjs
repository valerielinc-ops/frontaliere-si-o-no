#!/usr/bin/env node
/**
 * elprom job parser — Fetcher and job builder.
 *
 * Source: https://elprom.ch/job/elektroplaner-3d-bim-m-w-d-80-100-wallisellen/
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllElpromJobs()  — Fetch and parse all jobs
 *   - isElpromJob()         — Match jobs belonging to this company
...
export const ELPROM_KEY = 'elprom';
export const ELPROM_COMPANY_NAME = 'elprom';
export const ELPROM_COMPANY_DOMAIN = 'elprom.ch';

const CAREER_URL = 'https://elprom.ch/job/elektroplaner-3d-bim-m-w-d-80-100-wallisellen/';
...
/**
 * Check if a job belongs to elprom.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isElpromJob(job) {
  ...
  return (
    key === ELPROM_KEY ||
    key.startsWith('elprom-') ||
    company.includes('elprom') ||
    url.includes('elprom.ch')
  );
}

/**
 * Validate that a URL belongs to elprom's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'elprom.ch' || host.endsWith('.elprom.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(ELPROM_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all elprom jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllElpromJobs() {
  console.log(`🔍 Fetching elprom jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} elprom ch`);
    ...
    const job = {
      ...
      company: ELPROM_COMPANY_NAME,
      companyKey: ELPROM_KEY,
      companyDomain: ELPROM_COMPANY_DOMAIN,
      title,
  ...
  console.log(`\n📋 Total elprom jobs discovered: ${jobs.length}`);
