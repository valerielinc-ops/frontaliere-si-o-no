...
async function fetchJobListings() {
  const spec = loadSpec(RECRUITINGAPP_2677_KEY);
 ...
 */
export async function fetchAllRecruitingapp2677Jobs() {
  ...
  for (const listing of listings) {
    ...
    const job = {
      ...
      description: descriptionText || `${title} — E-Recruiting LLB-Gruppe Stellen`,
      descriptionByLocale: { [sourceLang]: descriptionText || `${title} — E-Recruiting LLB-Gruppe Stellen` },
      location,
      canton,
      url: publicUrl,
