...
function detectCategory(title = '') {
  ...
  if (/\b(lernende|lehrstelle|praktik|apprenti|stages?(?=\W|$))/.test(t)) return 'Formazione';
  return 'Sanità / Ospedali';
}

function detectExperienceLevel(title = '') {
  const t = normalize(title);
  if (/\b(lernende|lehrstelle|praktik|stages?(?=\W|$)|intern(?:ship)?s?(?=\W|$)|apprenti)/.test(t)) return 'intern';
