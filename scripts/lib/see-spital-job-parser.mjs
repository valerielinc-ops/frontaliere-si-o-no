#!/usr/bin/env node
/**
 * See-Spital job parser — Umantis ATS (tenant 2939).
 *
 * Listing page: https://recruitingapp-2939.umantis.com/Jobs/All?lang=ger
 *   - Same Umantis 2023 server-rendered HTML layout used by Inselspital
 *     (tenant 2624) and Spital Männedorf (tenant 2736).
 *     `<tr class="table-as-list__contentrow1|2">` rows with
 *     `tableaslist_element_*` children. Element IDs are stable across tenants:
 ...
 * and snippet as source of truth.
 *
 * Public landing wrapper: https://see-spital.ch/jobs/ (links out to Umantis).
 *
 * See-Spital operates two campuses (Horgen + Kilchberg). HQ Horgen, ZH 8810.
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllSeeSpitalJobs() — Fetch and parse all jobs across pages
 *   - isSeeSpitalJob()        — Match jobs belonging to this company
...
export function isSeeSpitalJob(job) {
  ...
  return (
    key === SEE_SPITAL_KEY ||
    key.startsWith('see-spital') ||
    company.includes('see-spital') ||
    company.includes('see spital') ||
    url.includes('see-spital.ch') ||
...
export function parseSeeSpitalListingPage(html = '') {
  const results = [];
  const seen = new Set();
...
export async function fetchAllSeeSpitalJobs() {
  console.log(`🏥 Fetching ${SEE_SPITAL_COMPANY_NAME} jobs`);
  ...
  const firstListings = parseSeeSpitalListingPage(firstHtml);
  ...
  for (const listing of allListings) {
    ...
    const urlHash = createHash('sha1')
      .update(`${SEE_SPITAL_KEY}-vacancy-${listing.vacancyId}`)
      .digest('hex')
      .slice(0, 12);
    ...
    const job = {
      // ── Required fields ──
      id: `${SEE_SPITAL_KEY}-${urlHash}`,
      slug: jobSlug,
      slugByLocale: { [sourceLang]: jobSlug },
      company: SEE_SPITAL_COMPANY_NAME,
      companyKey: SEE_SPITAL_KEY,
      companyDomain: SEE_SPITAL_COMPANY_DOMAIN,
      title,
  ...
  if (jobs.length === 0 && quarantinedDeadDetail > 0) {
    console.warn(`  ⚠️  ${SEE_SPITAL_COMPANY_NAME}: ALL listings have a deprecated (cross-host-redirecting) Umantis detail URL — source appears to have migrated off Umantis. Emitting 0 jobs (no hard failure). Follow-up: per-tenant public-site/Prospective description extraction (issue #1245).`);
  }
  console.log(`\n📋 Total ${SEE_SPITAL_COMPANY_NAME} jobs discovered: ${jobs.length} (${detailHits}/${jobs.length} with rich detail content${quarantinedDeadDetail > 0 ? `, ${quarantinedDeadDetail} quarantined dead-detail` : ''})`);
  return jobs;
}
