...
function detectCategory(title = '') {
  ...
  if (/\b(lehrling|lernend|apprenti|praktik|intern(?:ship)?s?(?![a-zA-Z0-9_À-ÖØ-öø-ÿ])|studium|studi[oe])/.test(t)) return 'Formazione';
  return 'Altro';
}

function detectExperienceLevel(title = '') {
  const t = normalize(title);
  if (/\b(lehrling|lernend|apprenti|praktik|intern(?:ship)?s?(?![a-zA-Z0-9_À-ÖØ-öø-ÿ])|trainee|aushilfe|studium|lehrstelle|studi[oe])/.test(t)) return 'intern';
...
function detectEmploymentType(title = '', srType = '') {
  ...
  if (/\b(praktik|intern(?:ship)?s?(?![a-zA-Z0-9_À-ÖØ-öø-ÿ])|stages?(?![a-zA-Z0-9_À-ÖØ-öø-ÿ])|lehrling|lernend|studium)/.test(t)) return 'INTERN';
