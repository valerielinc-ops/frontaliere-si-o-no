...
function detectCategory(title = '') {
  ...
  if (/\b(lehrling|lernend|apprenti|praktik|intern(?:ship)?s?(?=\W|$)|studium|studi[oe])/.test(t)) return 'Formazione';
  return 'Altro';
}

function detectExperienceLevel(title = '') {
  const t = normalize(title);
  if (/\b(lehrling|lernend|apprenti|praktik|intern(?:ship)?s?(?=\W|$)|trainee|aushilfe|studium|lehrstelle|studi[oe])/.test(t)) return 'intern';
...
function detectEmploymentType(title = '', srType = '') {
  ...
  if (/\b(praktik|intern(?:ship)?s?(?=\W|$)|stages?(?=\W|$)|lehrling|lernend|studium)/.test(t)) return 'INTERN';
