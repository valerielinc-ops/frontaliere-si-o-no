#!/usr/bin/env node
/**
 * ATEC Personal AG job parser — Fetcher and job builder.
 *
 * Source: https://atec-personal.ch/stellenangebot/prozessverantwortlicher-pruef-und-messmittel-80-100-a-3186-duedingen/cac94c1f-e85d-40e7-814b-e9f55ab36b29/
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllAtecPersonalJobs()  — Fetch and parse all jobs
 *   - isAtecPersonalJob()         — Match jobs belonging to this company
...
export const ATEC_PERSONAL_KEY = 'atec-personal';
export const ATEC_PERSONAL_COMPANY_NAME = 'ATEC Personal AG';
export const ATEC_PERSONAL_COMPANY_DOMAIN = 'atec-personal.ch';

const CAREER_URL = 'https://atec-personal.ch/stellenangebot/prozessverantwortlicher-pruef-und-messmittel-80-100-a-3186-duedingen/cac94c1f-e85d-40e7-814b-e9f55ab36b29/';
...
/**
 * Check if a job belongs to ATEC Personal AG.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isAtecPersonalJob(job) {
  ...
  return (
    key === ATEC_PERSONAL_KEY ||
    key.startsWith('atec-personal-') ||
    company.includes('atec personal ag') ||
    url.includes('atec-personal.ch')
  );
}

/**
 * Validate that a URL belongs to ATEC Personal AG's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'atec-personal.ch' || host.endsWith('.atec-personal.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(ATEC_PERSONAL_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all ATEC Personal AG jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllAtecPersonalJobs() {
  console.log(`🔍 Fetching ATEC Personal AG jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} atec-personal ch`);
    const urlHash = createHash('sha1').update(publicUrl).digest('hex').slice(0, 12);

    const job = {
      // ── Required fields ──
      id: `atec-personal-${urlHash}`,
      slug: jobSlug,
      slugByLocale: { [sourceLang]: jobSlug },
      company: ATEC_PERSONAL_COMPANY_NAME,
      companyKey: ATEC_PERSONAL_KEY,
      companyDomain: ATEC_PERSONAL_COMPANY_DOMAIN,
      title,
  ...
  console.log(`\n📋 Total ATEC Personal AG jobs discovered: ${jobs.length}`);
  return jobs;
}
