#!/usr/bin/env node
/**
 * FISBA AG job parser — Fetcher and job builder.
 *
 * Source: https://fisba.com/en/current-vacancies
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllFisbaJobs()  — Fetch and parse all jobs
 *   - isFisbaJob()         — Match jobs belonging to this company
...
export const FISBA_KEY = 'fisba';
export const FISBA_COMPANY_NAME = 'FISBA AG';
export const FISBA_COMPANY_DOMAIN = 'fisba.com';

const CAREER_URL = 'https://fisba.com/en/current-vacancies';
...
/**
 * Check if a job belongs to FISBA AG.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isFisbaJob(job) {
  ...
  return (
    key === FISBA_KEY ||
    key.startsWith('fisba-') ||
    company.includes('fisba ag') ||
    url.includes('fisba.com')
  );
}

/**
 * Validate that a URL belongs to FISBA AG's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'fisba.com' || host.endsWith('.fisba.com');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(FISBA_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all FISBA AG jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllFisbaJobs() {
  console.log(`🔍 Fetching FISBA AG jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} fisba ch`);
    ...
    const job = {
      ...
      company: FISBA_COMPANY_NAME,
      companyKey: FISBA_KEY,
      companyDomain: FISBA_COMPANY_DOMAIN,
      title,
  ...
  console.log(`\n📋 Total FISBA AG jobs discovered: ${jobs.length}`);
  return jobs;
}
