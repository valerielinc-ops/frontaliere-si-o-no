...
function detectCategory(title = '') {
  ...
  if (/\b(praktik|intern(?:ship)?s?(?=\W|$)|werkstudent|lehrling|lernend|apprenti)/.test(t)) return 'Formazione';
  return SECTOR === 'Industria / Manifattura' ? 'Produzione' : 'Altro';
}

function detectExperienceLevel(title = '') {
  const t = normalize(title);
  if (/\b(praktik|stages?(?=\W|$)|intern(?:ship)?s?(?=\W|$)|werkstudent|lehrling|lernend|apprenti)/.test(t)) return 'intern';
