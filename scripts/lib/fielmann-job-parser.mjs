...
function detectExperienceLevel(title = '') {
  const t = normalize(title);
  if (/\b(praktik|stages?(?![a-zA-Z0-9_À-ÖØ-öø-ÿ])|stagiair|intern(?:ship)?s?(?![a-zA-Z0-9_À-ÖØ-öø-ÿ])|apprendist|lehrling|lernend|apprenti|ausbildung|trainee)/.test(t)) return 'intern';
...
export async function fetchAllFielmannJobs() {
  ...
  for (const listing of listings) {
    ...
    const cityFromVacancyText = locInfo.city
      ? '' : rescueSwissCityFromText(stripHtml(info.jobDescription || ''));
    const city = locInfo.city || cityFromVacancyText;
    if (!city) {
