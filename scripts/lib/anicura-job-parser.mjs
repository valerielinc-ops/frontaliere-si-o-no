#!/usr/bin/env node
/**
 * anicura job parser — Fetcher and job builder.
 *
 * Source: https://jobs.anicura.ch/jobs/5851151-tieraerzt-in-80-100-w-m-d-thun
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllAnicuraJobs()  — Fetch and parse all jobs
 *   - isAnicuraJob()         — Match jobs belonging to this company
...
export const ANICURA_KEY = 'anicura';
export const ANICURA_COMPANY_NAME = 'anicura';
export const ANICURA_COMPANY_DOMAIN = 'jobs.anicura.ch';

const CAREER_URL = 'https://jobs.anicura.ch/jobs/5851151-tieraerzt-in-80-100-w-m-d-thun';
...
/**
 * Check if a job belongs to anicura.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isAnicuraJob(job) {
  ...
  return (
    key === ANICURA_KEY ||
    key.startsWith('anicura-') ||
    company.includes('anicura') ||
    url.includes('jobs.anicura.ch')
  );
}

/**
 * Validate that a URL belongs to anicura's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'jobs.anicura.ch' || host.endsWith('.jobs.anicura.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(ANICURA_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all anicura jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllAnicuraJobs() {
  console.log(`🔍 Fetching anicura jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} anicura ch`);
    ...
    const job = {
      ...
      company: ANICURA_COMPANY_NAME,
      companyKey: ANICURA_KEY,
      companyDomain: ANICURA_COMPANY_DOMAIN,
      title,
  ...
  console.log(`\n📋 Total anicura jobs discovered: ${jobs.length}`);
  return jobs;
}
