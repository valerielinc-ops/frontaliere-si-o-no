 ...
 *
 * Source: https://novelis.wd5.myworkdayjobs.com/NovelisExternalCareerSite
 *
...
const WORKDAY_API_BASE = 'https://novelis.wd5.myworkdayjobs.com/wday/cxs/novelis/NovelisExternalCareerSite';
const WORKDAY_PUBLIC_BASE = 'https://novelis.wd5.myworkdayjobs.com/en/NovelisExternalCareerSite';
const WORKDAY_HOST = 'novelis.wd5.myworkdayjobs.com';
...
function inferCanton(location = '') {
  ...
  if (loc.includes('sierre') || loc.includes('sion') || loc.includes('visp') || loc.includes('viège')) return 'VS';
  ...
  return 'VS'; // Default to Valais — Novelis's main Swiss site is Sierre
}

/* ── Main Fetch Function ─────────────────────────────────── */

/**
 * Fetch all Novelis Swiss jobs from the Workday API.
 * Returns ParsedJob[] with source-locale fields only.
 */
export async function fetchAllNovelisJobs() {
  console.log(`\ud83d\udd0d Fetching Novelis jobs from Workday API`);
  ...
  for (const listing of listings) {
    ...
    const descText = descriptionText
      ...
      : `${title} position at Novelis in ${city}, Switzerland.\n\nNovelis is the world leader in aluminium rolling and recycling, with a major production facility in Sierre (Valais), Switzerland. The company produces flat-rolled aluminium products for the automotive, beverage can, and specialty markets.`.trim();

    const sourceLang = detectLang(descriptionText || title, 'en');
    const jobSlug = slugify(`${title} novelis ch`);
    ...
    const job = {
      ...
      url: publicUrl,
      applyUrl: publicUrl,
      source: 'Novelis Dedicated Parser (Workday)',
      sourceLang,
  ...
  console.log(`\n\ud83d\udccb Total unique Novelis jobs discovered: ${jobs.length}`);
  return jobs;
}
