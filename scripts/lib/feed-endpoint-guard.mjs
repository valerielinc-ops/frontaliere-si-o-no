 ...
 *    diff in this repo can fix. Errors carrying `feedEndpointUnavailable` are
 *    therefore soft-exited by `crawler-template.mjs` (pipeline catch +
 *    `exitCrawlerOnError`) exactly like a connection-level failure: existing
 *    slice preserved, run green, and persistence still surfaces through the
 *    crawler-health monitor (3 consecutive zero-job runs → broken issue,
