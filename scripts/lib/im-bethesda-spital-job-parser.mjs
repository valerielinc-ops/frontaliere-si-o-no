#!/usr/bin/env node
/**
 * & im Bethesda Spital job parser — Fetcher and job builder.
 *
 * Source: https://www.bethesda-spital.ch/de/ueber-uns/karriere/jobs.html
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllImBethesdaSpitalJobs()  — Fetch and parse all jobs
 *   - isImBethesdaSpitalJob()         — Match jobs belonging to this company
...
export const IM_BETHESDA_SPITAL_KEY = 'im-bethesda-spital';
export const IM_BETHESDA_SPITAL_COMPANY_NAME = '& im Bethesda Spital';
export const IM_BETHESDA_SPITAL_COMPANY_DOMAIN = 'bethesda-spital.ch';

const CAREER_URL = 'https://www.bethesda-spital.ch/de/ueber-uns/karriere/jobs.html';
...
/**
 * Check if a job belongs to & im Bethesda Spital.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isImBethesdaSpitalJob(job) {
  ...
  return (
    key === IM_BETHESDA_SPITAL_KEY ||
    key.startsWith('im-bethesda-spital-') ||
    company.includes('& im bethesda spital') ||
    url.includes('bethesda-spital.ch')
  );
}

/**
 * Validate that a URL belongs to & im Bethesda Spital's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'bethesda-spital.ch' || host.endsWith('.bethesda-spital.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(IM_BETHESDA_SPITAL_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all & im Bethesda Spital jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllImBethesdaSpitalJobs() {
  console.log(`🔍 Fetching & im Bethesda Spital jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} im-bethesda-spital ch`);
    const urlHash = createHash('sha1').update(publicUrl).digest('hex').slice(0, 12);

    const job = {
      // ── Required fields ──
      id: `im-bethesda-spital-${urlHash}`,
      slug: jobSlug,
      slugByLocale: { [sourceLang]: jobSlug },
      company: IM_BETHESDA_SPITAL_COMPANY_NAME,
      companyKey: IM_BETHESDA_SPITAL_KEY,
      companyDomain: IM_BETHESDA_SPITAL_COMPANY_DOMAIN,
      title,
  ...
  console.log(`\n📋 Total & im Bethesda Spital jobs discovered: ${jobs.length}`);
  return jobs;
}
