...
// ISO 3166-1 alpha-3 codes from the same snapshot. Workday sometimes emits a
// three-letter country prefix (for example `CHE - Lugano`), but a bare
// two/three-letter location is not necessarily a country code (`NY` is a
// legitimate city/site token). Keep the distinction in one inventory instead
// of teaching each ATS parser a different allowlist.
const ISO_ALPHA3_SNAPSHOT = `
AFG ALA ALB DZA ASM AND AGO AIA ATA ATG ARG ARM ABW AUS AUT AZE BHS BHR BGD BRB BLR BEL BLZ BEN BMU BTN BOL BES BIH BWA
BVT BRA IOT VGB BRN BGR BFA BDI CPV KHM CMR CAN CYM CAF TCD CHL CHN HKG MAC CXR CCK COL COM COG COK CRI HRV CUB CUW CYP
CZE PRK COD DNK DJI DMA DOM ECU EGY SLV GNQ ERI EST SWZ ETH FLK FRO FJI FIN FRA GUF PYF ATF GAB GMB GEO DEU GHA GIB GRC
GRL GRD GLP GUM GTM GGY GIN GNB GUY HTI HMD VAT HND HUN ISL IND IDN IRN IRQ IRL IMN ISR ITA CIV JAM JPN JEY JOR KAZ KEN
KIR KWT KGZ LAO LVA LBN LSO LBR LBY LIE LTU LUX MDG MWI MYS MDV MLI MLT MHL MTQ MRT MUS MYT MEX FSM MCO MNG MNE MSR
MAR MOZ MMR NAM NRU NPL NLD NCL NZL NIC NER NGA NIU NFK MNP MKD NOR OMN PAK PLW PAN PNG PRY PER PHL PCN POL PRT PRI QAT
KOR MDA REU ROU RUS RWA BLM SHN KNA LCA MAF SPM VCT WSM SMR STP SAU SEN SRB SYC SLE SGP SXM SVK SVN SLB SOM ZAF SGS SSD
ESP LKA PSE SDN SUR SJM SWE CHE SYR TWN TJK THA TLS TGO TKL TON TTO TUN TUR TKM TCA TUV UGA UKR ARE GBR TZA UMI USA VIR URY
UZB VUT VEN VNM WLF ESH YEM ZMB ZWE
`.trim().split(/\s+/);

export const ISO_ALPHA3_COUNTRY_CODES = new Set(ISO_ALPHA3_SNAPSHOT);
