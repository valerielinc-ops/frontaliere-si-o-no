...
export function mergeSpitalDavosListing(existing = {}, incoming = {}) {
  return mergeUmantisListing(existing, incoming, 'Spital Davos');
}
...
function detectExperienceLevel(title = '', entryLevel = '') {
  ...
  const signal = `${t} ${e}`;
  if (/\b(praktik|stages?(?![a-zA-Z0-9_À-ÖØ-öø-ÿ])|stagiair|intern(?:ship)?s?(?![a-zA-Z0-9_À-ÖØ-öø-ÿ])|apprendist|lehrling|lernend|apprenti)/.test(signal)) return 'intern';
