...
export async function fetchAllOkjobJobs() {
  ...
  for (const listing of listings) {
    ...
    const job = {
      ...
      description: descriptionText || `${title} — OK Job SA, succursale di Mendrisio`,
      descriptionByLocale: { [sourceLang]: descriptionText || `${title} — OK Job SA, succursale di Mendrisio` },
      location,
      canton,
      url: publicUrl,
