#!/usr/bin/env node
/**
 * Stellenpartner AG job parser — Fetcher and job builder.
 *
 * Source: https://www.stellenpartner.ch/stellen/
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllStellenpartnerJobs()  — Fetch and parse all jobs
 *   - isStellenpartnerJob()         — Match jobs belonging to this company
...
export const STELLENPARTNER_KEY = 'stellenpartner';
export const STELLENPARTNER_COMPANY_NAME = 'Stellenpartner AG';
export const STELLENPARTNER_COMPANY_DOMAIN = 'stellenpartner.ch';

const CAREER_URL = 'https://www.stellenpartner.ch/stellen/';
...
/**
 * Check if a job belongs to Stellenpartner AG.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isStellenpartnerJob(job) {
  ...
  return (
    key === STELLENPARTNER_KEY ||
    key.startsWith('stellenpartner') ||
    company.includes('stellenpartner ag') ||
    url.includes('stellenpartner.ch')
  );
}

/**
 * Validate that a URL belongs to Stellenpartner AG's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'stellenpartner.ch' || host.endsWith('.stellenpartner.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(STELLENPARTNER_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all Stellenpartner AG jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllStellenpartnerJobs() {
  console.log(`🔍 Fetching Stellenpartner AG jobs`);
  ...
  for (const listing of listings) {
    ...
    const job = {
      // ── Required fields ──
      id: `stellenpartner-${urlHash}`,
      slug: jobSlug,
      slugByLocale: { [sourceLang]: jobSlug },
      company: STELLENPARTNER_COMPANY_NAME,
      companyKey: STELLENPARTNER_KEY,
      companyDomain: STELLENPARTNER_COMPANY_DOMAIN,
      title,
  ...
  console.log(`\n📋 Total Stellenpartner AG jobs discovered: ${jobs.length}`);
  return jobs;
}
