#!/usr/bin/env node
/**
 * Stellentreff AG job parser — Fetcher and job builder.
 *
 * Source: https://www.stellentreff.ch/stellen/
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllStellentreffJobs()  — Fetch and parse all jobs
 *   - isStellentreffJob()         — Match jobs belonging to this company
...
export const STELLENTREFF_KEY = 'stellentreff';
export const STELLENTREFF_COMPANY_NAME = 'Stellentreff AG';
export const STELLENTREFF_COMPANY_DOMAIN = 'stellentreff.ch';

const CAREER_URL = 'https://www.stellentreff.ch/stellen/';
...
/**
 * Check if a job belongs to Stellentreff AG.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isStellentreffJob(job) {
  ...
  return (
    key === STELLENTREFF_KEY ||
    key.startsWith('stellentreff') ||
    company.includes('stellentreff ag') ||
    url.includes('stellentreff.ch')
  );
}

/**
 * Validate that a URL belongs to Stellentreff AG's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'stellentreff.ch' || host.endsWith('.stellentreff.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(STELLENTREFF_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all Stellentreff AG jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllStellentreffJobs() {
  console.log(`🔍 Fetching Stellentreff AG jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} stellentreff ch`);
    const urlHash = createHash('sha1').update(publicUrl).digest('hex').slice(0, 12);

    const job = {
      // ── Required fields ──
      id: `stellentreff-${urlHash}`,
      slug: jobSlug,
      slugByLocale: { [sourceLang]: jobSlug },
      company: STELLENTREFF_COMPANY_NAME,
      companyKey: STELLENTREFF_KEY,
      companyDomain: STELLENTREFF_COMPANY_DOMAIN,
      title,
