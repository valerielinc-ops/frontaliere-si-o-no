#!/usr/bin/env node
/**
 * ParMag AG job parser — Fetcher and job builder.
 *
 * Source: https://parmag.ch/stellen
 *
 * Exports the 4 required functions for the crawler template:
 *   - fetchAllParmagJobs()  — Fetch and parse all jobs
 *   - isParmagJob()         — Match jobs belonging to this company
...
export const PARMAG_KEY = 'parmag';
export const PARMAG_COMPANY_NAME = 'ParMag AG';
export const PARMAG_COMPANY_DOMAIN = 'parmag.ch';

const CAREER_URL = 'https://parmag.ch/stellen';
...
/**
 * Check if a job belongs to ParMag AG.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isParmagJob(job) {
  ...
  return (
    key === PARMAG_KEY ||
    key.startsWith('parmag-') ||
    company.includes('parmag ag') ||
    url.includes('parmag.ch')
  );
}

/**
 * Validate that a URL belongs to ParMag AG's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'parmag.ch' || host.endsWith('.parmag.ch');
  } catch {
    return false;
  }
}
...
async function fetchJobListings() {
  const spec = loadSpec(PARMAG_KEY);
  return runSpecInProduction(spec);
}

/**
 * Fetch all ParMag AG jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllParmagJobs() {
  console.log(`🔍 Fetching ParMag AG jobs`);
  ...
  for (const listing of listings) {
    ...
    const jobSlug = slugify(`${title} ${location} parmag ch`);
    ...
    const job = {
      ...
      company: PARMAG_COMPANY_NAME,
      companyKey: PARMAG_KEY,
      companyDomain: PARMAG_COMPANY_DOMAIN,
      title,
      ...
      canton,
      url: publicUrl,
      source: 'ParMag AG Dedicated Parser',
      sourceLang,
  ...
  console.log(`\n📋 Total ParMag AG jobs discovered: ${jobs.length}`);
  return jobs;
}
