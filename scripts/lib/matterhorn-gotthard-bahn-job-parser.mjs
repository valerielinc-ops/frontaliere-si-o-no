#!/usr/bin/env node
/**
 * Matterhorn Gotthard Bahn job parser — Rexx Systems HTML scraper + JSON-LD.
 *
 * Source: https://jobs.bvzholding.ch/
 *
 * Strategy:
 ...
 *   - fetchAllMatterhornGotthardBahnJobs()  — Fetch and parse all jobs
 *   - isMatterhornGotthardBahnJob()         — Match jobs belonging to this company
...
export const MATTERHORN_GOTTHARD_BAHN_KEY = 'matterhorn-gotthard-bahn';
export const MATTERHORN_GOTTHARD_BAHN_COMPANY_NAME = 'Matterhorn Gotthard Bahn';
export const MATTERHORN_GOTTHARD_BAHN_COMPANY_DOMAIN = 'bvzholding.ch';

const CAREER_URL = 'https://jobs.bvzholding.ch/';
const BASE_URL = 'https://jobs.bvzholding.ch';
...
/**
 * Check if a job belongs to Matterhorn Gotthard Bahn.
 * Used by the template to filter this company's jobs from the global dataset.
 */
export function isMatterhornGotthardBahnJob(job) {
  ...
  return (
    key === MATTERHORN_GOTTHARD_BAHN_KEY ||
    key.startsWith('matterhorn-gotthard-bahn') ||
    company.includes('matterhorn gotthard bahn') ||
    url.includes('bvzholding.ch')
  );
}

/**
 * Validate that a URL belongs to Matterhorn Gotthard Bahn's domain.
 */
export function isTrustedDomain(rawUrl = '') {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === 'bvzholding.ch' || host.endsWith('.bvzholding.ch');
  } catch {
    return false;
  }
}
...
function detectCategory(title = '', description = '') {
  ...
  if (/\b(lokf[uü]hr|triebfahrzeug|zug|rangi|train|driver)/.test(t)) return 'Trasporti';
  if (/\b(ingegner|engineer|entwickl)/.test(t)) return 'Ingegneria';
  if (/\b(techni|tecnic|mecanic|elektr|install|instandhalt|fahrba|bahnstrom)/.test(t)) return 'Tecnica';
  if (/\b(intervention|einsatzleit|sicherheit|safety)/.test(t)) return 'Sicurezza';
  if (/\b(admin|segret|contab|buchhalt|account)/.test(t)) return 'Amministrazione';
  if (/\b(vendita|sales|verkauf|commerce)/.test(t)) return 'Commerciale';
  if (/\b(logist|magazz|lager|warehouse)/.test(t)) return 'Logistica';
  if (/\b(it|software|develop|programm|support.?tech)/.test(t)) return 'IT';
  if (/\b(hr|human|risorse|personal)/.test(t)) return 'Risorse Umane';
  if (/\b(market|kommunik|comunicaz|praktik)/.test(t)) return 'Marketing';
...
function inferPostalCode(locality = '') {
  const key = normalize(locality)
    ...
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z]+/g, '');
  for (const [name, code] of Object.entries(VS_POSTAL_CODES)) {
    const normName = name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z]+/g, '');
    if (key.includes(normName) || normName.includes(key)) return code;
  }
  return '3900'; // Default to Brig (MGB headquarters)
}
...
function extractLocation(jsonLd = {}) {
  ...
  if (locations.length === 0) {
    return { locality: 'Brig', postalCode: '3900', streetAddress: 'Bahnhofstrasse 7', canton: 'VS' };
  }

  // Use the first location as primary
  const primary = locations[0];
  const address = primary?.address || {};
  const locality = normalizeSpace(address.addressLocality || 'Brig');
  ...
  // Infer canton from region or locality
  let canton = 'VS';
  if (region) {
    const regionLower = region.toLowerCase();
    if (regionLower.includes('uri')) canton = 'UR';
    else if (regionLower.includes('graubünden') || regionLower.includes('grisons')) canton = 'GR';
...
function buildJobFromJsonLd(jsonLd, detailUrl, rexxId) {
  ...
  return {
    // ── Required fields ──
    id: `${MATTERHORN_GOTTHARD_BAHN_KEY}-${urlHash}`,
    slug: jobSlug,
    slugByLocale: { [sourceLang]: jobSlug },
    company: MATTERHORN_GOTTHARD_BAHN_COMPANY_NAME,
    companyKey: MATTERHORN_GOTTHARD_BAHN_KEY,
    companyDomain: MATTERHORN_GOTTHARD_BAHN_COMPANY_DOMAIN,
    title,
    titleByLocale: { [sourceLang]: title },
    description: descriptionText || `${title} — ${MATTERHORN_GOTTHARD_BAHN_COMPANY_NAME}`,
    descriptionByLocale: { [sourceLang]: descriptionText || `${title} — ${MATTERHORN_GOTTHARD_BAHN_COMPANY_NAME}` },
    location: location.locality,
    canton: location.canton,
    url: detailUrl,
    source: 'Matterhorn Gotthard Bahn Dedicated Parser (Rexx Systems)',
    sourceLang,
...
/**
 * Fetch all Matterhorn Gotthard Bahn jobs.
 * Returns an array of ParsedJob objects (source-locale only).
 *
 * IMPORTANT: Only set source-locale fields. Other locales are filled
 * by the AI localization step and translate-pending pipeline.
 */
export async function fetchAllMatterhornGotthardBahnJobs() {
  console.log(`🔍 Fetching Matterhorn Gotthard Bahn jobs`);
  ...
  console.log(`\n📋 Total Matterhorn Gotthard Bahn jobs discovered: ${deduped.length}`);
  return deduped;
}
