...
function detectExperienceLevel(title = '', experience = '') {
  ...
  if (/\b(praktik|stages?(?=\W|$)|stagiair|intern(?:ship)?s?(?=\W|$)|apprendist|lehrling|lernend|apprenti|studentisch)/.test(t)) return 'intern';
...
function detectEmploymentType(empType = '') {
  ...
  if (t.includes('praktikum') || t.includes('studentisch') || /\bintern(?:ship)?s?(?=\W|$)/.test(t)) return 'INTERN';
  return 'FULL_TIME';
}
