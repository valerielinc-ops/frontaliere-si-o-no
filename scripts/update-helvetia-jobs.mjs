...
import {
  fetchHelvetiaJobUrls,
  fetchHelvetiaDetailPage,
...
const COMPANY_KEY = 'helvetia';
const COMPANY_NAME = 'Helvetia Assicurazioni';

function isCompanyJob(job) {
  const key = String(job?.companyKey || job?.company || '').toLowerCase();
  const url = String(job?.url || '').toLowerCase();
  return key.includes('helvetia') || url.includes('helvetia.com');
}
...
async function main() {
  setCrawlerStartTime();
  registerCrawlerSummaryGuard(COMPANY_KEY, 'Helvetia');
  ...
  const rawJobs = await fetchHelvetiaJobUrls();
  if (rawJobs.length === 0) {
    console.log('\u26a0\ufe0f No jobs found on Helvetia careers page. Keeping existing jobs.');
    return;
  }

  console.log(`\ud83e\udde9 Found ${rawJobs.length} Helvetia job links. Fetching details...`);
  const parsedJobs = [];
  for (const raw of rawJobs) {
    const detail = await fetchHelvetiaDetailPage(raw.url);
    ...
    const jobSlug = slugify(`${raw.title}-helvetia-${raw.location}`);
    parsedJobs.push({
      id: `helvetia-${urlHash}`,
      slug: jobSlug,
      ...
      url: raw.url,
      source: 'Helvetia Dedicated Parser',
  ...
  printPublishedJobUrls(published, 'Helvetia');
  writeJobsSummary(published, 'Helvetia');

  const afterSnapshot = snapshotJobSlugs(published);
  const diff = computeCrawlDiff(_beforeSnapshot, afterSnapshot);
  printCrawlChangeSummary(diff, 'Helvetia');
  writeCrawlChangeSummaryToGH(diff, 'Helvetia');
  ...
  writeSummaryCrawlerSlice({ key: COMPANY_KEY, label: 'Helvetia', generatedAt: new Date().toISOString(), total: _sliceJobs.length, newCount: diff.newJobs.length, updatedCount: diff.updatedJobs.length, removedCount: diff.removedJobs.length, unchangedCount: diff.unchangedCount, durationMs: _durationMs, avgDurationMs: _durationMs, durationHistory: [_durationMs], newJobs: diff.newJobs.slice(0, 30), updatedJobs: diff.updatedJobs.slice(0, 30), removedJobs: diff.removedJobs.slice(0, 30), unchangedJobs: _sliceJobs.slice(0, 30) });
  await assembleJobsDataset();
}

main().catch((err) => { console.error(`\u274c Helvetia crawler failed: ${err?.message || err}`); process.exit(1); });
