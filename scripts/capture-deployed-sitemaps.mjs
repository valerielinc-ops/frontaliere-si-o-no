 ...
 * actually moved.
 *
 * Discovery: starts from sitemap.xml — the real, robots.txt-declared master
 * index that `sitemapAliasPlugin` regenerates every build from EVERY
 * `dist/sitemap-*.xml` file present — so the file list stays in sync with
 * build-plugins (no hardcoded list to drift from). Falls back to a curated
 * short list if the index isn't reachable (first deploy, etc).
 *
 * NB: this must NOT point at `sitemap-index.xml` — that's a separate,
 * orphaned legacy index (job-canton shards only, written by
 * `scripts/lib/sitemap-shard.mjs`) never referenced by robots.txt (#4395).
 * Discovering from it would silently narrow the captured snapshot to just
 * the job shards, dropping blog/pages/glossario/news/seo-hubs and every
 * other sub-sitemap from the pre-deploy diff.
 *
...
/**
 * Discover the list of sitemap-*.xml files by reading the real master
 * sitemap.xml index (robots.txt-declared, regenerated by
 * `sitemapAliasPlugin` from every `dist/sitemap-*.xml` file — NOT the
 * orphaned `sitemap-index.xml`, see module docstring above).
...
async function discoverSitemapFiles() {
  const indexXml = await fetchText(`https://${HOST}/sitemap.xml`);
...
async function main() {
  ...
  if (!files || files.length === 0) {
    console.warn(`⚠️ Could not read sitemap.xml from https://${HOST}/ — falling back to curated list`);
