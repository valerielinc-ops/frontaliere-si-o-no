#!/usr/bin/env node
/**
 * Dedicated Cerbios-Pharma SA crawler runner.
 *
 * Source:
 *   https://www.e-lavoro.ch/node/91
 *
 * Cerbios-Pharma SA is a pharmaceutical CDMO based in Barbengo/Lugano,
 * specializing in API and ADC production for global pharma clients.
 *
 * This script:
...
function isCompanyJob(job) {
  ...
  return (
    key === COMPANY_KEY ||
    key.includes('cerbios-pharma') ||
    key.includes('cerbios') ||
    company.includes('cerbios-pharma') ||
    company.includes('cerbios') ||
    host === COMPANY_HOST ||
    host === 'e-lavoro.ch' || host === 'cerbios.swiss'
  );
}
...
function isTrustedDomain(rawUrl = '') {
  try {
    ...
    return host === COMPANY_HOST || host === 'e-lavoro.ch' || host === 'cerbios.swiss';
  } catch { return false; }
}
...
function updateAdapterConfig(seedUrls) {
  ...
  const seedMetaByUrl = {};
  for (const url of seedUrls) seedMetaByUrl[url] = { company: COMPANY_NAME, companyDomain: 'cerbios.swiss' };
  adapter = { ...adapter, companyKey: COMPANY_KEY, companyName: COMPANY_NAME, companyHost: COMPANY_HOST, enabled: true, priority: 10, crawlerModes: ['html'], seedUrls, seedMetaByUrl, notes: 'Pharma CDMO in Barbengo (Lugano). Jobs via e-lavoro.ch portal.', updatedAt: new Date().toISOString() };
...
async function main() {
  ...
  validateDedicatedLocaleCoverage({ strictEnvVar: 'JOBS_CERBIOS_PHARMA_STRICT', label: COMPANY_NAME, dataJobsPath: DATA_JOBS, isTargetJob: isCompanyJob, locales: LOCALES, isTrustedDomain, untrustedDomainReason: 'url_not_cerbios_domain', failWhenNoJobs: false, noJobsMessage: `No ${COMPANY_NAME} jobs found — the company may not have active openings.` });
