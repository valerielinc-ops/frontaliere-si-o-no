...
14. **Every static SSG page MUST use the SPA shell + hydration.** When a build plugin emits HTML to `dist/` (SEO landings, hub indexes, per-leaf static-overlay pages), it MUST wrap content via `build-plugins/shared/seoPageShell.ts` (`buildSeoPageHtml`). Standalone HTML without the shell renders without site nav/footer/theme/design-tokens/popup-newsletter/analytics/consent. The helper injects `<body class="bg-surface-alt text-heading overflow-x-hidden">`, the hashed entry JS/CSS, and `<div id="root"><main>` wrapper. Plugin contract: `apply: 'build'` + `enforce: 'post'` + emit in `closeBundle()`. Pass `distDir` to every `buildSeoPageHtml` call. For body styling use Tailwind utility classes inline — never custom class names that aren't in `index.css` (Tailwind purges them). **CRITICAL — Tailwind scan:** `tailwind.config.js` `content` array MUST include `./build-plugins/**/*.{js,ts}` (added 2026-05-07). Without this, Tailwind purges utilities and pages render unstyled. Test post-deploy with curl + Playwright snapshot. Exceptions: robots.txt, llms.txt, sitemap*.xml; 404/500/503 error pages.

    **SPA-over-static handoff (statica → crawler, utenti → SPA se disponibile):** the static body emitted by the shell is a *crawler-facing fallback*. Whether the user sees it or gets the hydrated SPA is driven entirely by `services/router.ts`:
    - If `parsePath(url).route.staticOverlay === true` → the URL has **no SPA equivalent** (fuel-daily/{station}, weekly-employers/{city}, health-premiums/{canton}, border-wait/{crossing}, jobs-observatory/{sector}, cost-of-living/{city}, salary-hub long-tail like `stipendio-netto-80000-chf`, known `seoLanding` aliases). App.tsx skips the React `<main>`; the static body owns the page; top nav + sub-tab nav + footer hydrate normally so the chrome stays interactive.
    - If `parsePath(url).route` resolves to a real sub-tab (e.g. `/calcola-stipendio/confronta-retribuzione-ral` → `calcolatoreSubTab: 'ral'`, `/verifica-congedo-parentale` → `parental-leave`) and the URL also ships a static fallback → App.tsx renders the React `<main>` AND a `useEffect` sets `display: none` on `main.seo-static-content`. End users see the interactive calculator; crawlers (no JS) still receive the original static HTML.

    Do NOT re-introduce DOM-based heuristics that force lite-shell when `main.seo-static-content` is present — `hooks/useNavigationState.ts` initializes `staticOverlay` from the router only. When you add a new SPA sub-tab whose slug is already shipped as a static SEO landing, no extra wiring is needed: the router's `REVERSE_*` slug maps win over the SEO landing branch, and the takeover useEffect hides the static body automatically. Regression coverage: `tests/app-lite-shell.test.tsx`, `tests/router.test.ts` ("SPA equivalent wins over static landing").
