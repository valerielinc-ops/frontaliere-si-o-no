  ...
  it('opener differs across bridge kinds within the same locale', () => {
    ...
    for (const locale of LOCALES) {
      const openers = KINDS.map((kind) => {
        const html = renderBridgePageProse({ locale, bridgeKind: kind });
        // First paragraph after the H2 (the opener). Accept either the legacy
        // inline `style="margin:0 0 14px"` form OR the post-2026-05-20
        // class-extract form (`class="s-clIDbe"` — generated by the codemod
        // that hashed the inline style to a short class in seo-static.css).
        const match = html.match(/<p (?:style="margin:0 0 14px"|class="(?:s-[A-Za-z0-9_-]+ ?)+")>([^<]+(?:<[^>]+>[^<]*<\/[^>]+>[^<]*)*)<\/p>/);
