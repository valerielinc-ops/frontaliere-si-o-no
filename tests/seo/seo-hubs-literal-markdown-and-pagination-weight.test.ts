...
describe('seoHubs — pagination ladder page-weight byte-shave', () => {
  ...
  it('buildThinCantonHubHtml uses a compact window on page 1 and after (#8016)', () => {
    const totalPages = 400;
    ...
    // Page-1 uses the same first/last/adjacent window as every other page.
    expect([...laddered(first)].sort((a, b) => a - b)).toEqual([2, totalPages]);
    expect(first).toContain('<strong>1</strong>');
    expect(first).toContain(
      `<a href="/cerca-lavoro-argovia/tutti/page-2/" rel="next">2</a>`,
    );
    expect(first).toContain(`Sfoglia tutte le pagine (3)`);
