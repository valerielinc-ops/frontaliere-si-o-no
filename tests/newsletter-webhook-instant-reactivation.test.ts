...
function createFakeDb(existingDocs: Record<string, Record<string, Record<string, unknown>>> = {}) {
  ...
  const makeCollection = (name: string): any => ({
    doc: (docId: string) => {
      ...
      // by maybeEscalateSoftBounce et al.) — Resend's webhook core wraps its
      // status read/write in a transaction (#3206 item 3); single-threaded
      // tests just need tx.get/tx.set delegating to the same doc.
      docRef.firestore = {
