...
describe('real-corpus invariant: every manifest crawler in exactly one committed crawler-group-*.yml', () => {
  ...
  // background-step canonical runtime identities the same way the generator
  // names them (`id: crawler-<canonical runtime identity>`) and diffs the
  // union against data/crawler-manifest.json's housekeeping scopes.
  it('canonical crawler identities == union of all crawler-group-*.yml background steps, each exactly once', () => {
    const REPO_ROOT = path.resolve(import.meta.dirname, '..');
    const WORKFLOWS_DIR = path.join(REPO_ROOT, '.github/workflows');
    const { manifest } = JSON.parse(fs.readFileSync(path.join(REPO_ROOT, 'data/crawler-manifest.json'), 'utf8'));
    const manifestStepIds = manifest.map((c: any) => crawlerWorkflowStepId(c));

    const files = fs.readdirSync(WORKFLOWS_DIR).filter((f) => /^crawler-group-\d+\.yml$/.test(f));
    expect(files.length).toBeGreaterThan(0);

    const occurrences = new Map<string, string[]>(); // identity -> file names it was found in
    for (const f of files) {
      const doc = YAML.parse(fs.readFileSync(path.join(WORKFLOWS_DIR, f), 'utf8'));
      const jobKey = Object.keys(doc.jobs)[0];
      ...
      for (const step of steps) {
        ...
        expect(match, `background step in ${f} has no canonical crawler id: ${JSON.stringify(step.id)}`).not.toBeNull();
    ...
    const missing = manifestStepIds
      .map((stepId) => stepId.replace(/^crawler-/, ''))
      .filter((identity) => !occurrences.has(identity));
    ...
    expect(duplicated, `crawlers present in more than one group: ${duplicated.map(([identity, files]) => `${identity} in [${files.join(', ')}]`).join('; ')}`).toEqual([]);

    const manifestIdentitySet = new Set(manifestStepIds.map((stepId) => stepId.replace(/^crawler-/, '')));
    const extraneous = [...occurrences.keys()].filter((identity) => !manifestIdentitySet.has(identity));
