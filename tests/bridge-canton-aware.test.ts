...
describe('Bridge page canton-aware UX', () => {
  ...
  describe('F5: slug-map carries id + canton for cross-canton bridge resolution', () => {
    ...
    it('JobBoard wires a cross-canton lazy fetch when bridgeTarget is in a different canton shard', () => {
      ...
      // is missing but a bridge target slug is set. The locale argument became
      // mandatory when the shards landed per-locale (S10a): shard records are
      // locale-flattened, so a canton-only address would hand a /de/ page the
      // Italian slugs.
      expect(jobBoardSrc).toMatch(/getJobMetaForSlug\(targetSlug\)/);
      expect(jobBoardSrc).toMatch(/fetchJobsForCanton\(cantonCode, locale\)/);
    ...
    it('JobBoard falls back to the slim locale index when the canton shard returns empty', () => {
      // Until 2026-08-07 this was permanent production reality: no build step
      // emitted /data/jobs-by-canton/, so every shard 404'd and this fallback
      // WAS the bridge path. The shard pipeline (S10a) now emits the files and
      // CANTON_SHARDS_ENABLED is true, so the shard normally answers — but the
      // fallback must stay: it is what covers a stale/propagating CDN copy, a
      // job whose canton disagrees between the slug map and the index, and any
      // deploy where the shard set has not published yet. Losing it would drop
      // a live, indexed job onto JobOrphanView.
