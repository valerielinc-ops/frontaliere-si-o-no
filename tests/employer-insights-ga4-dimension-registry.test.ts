...
import { EMPLOYER_INSIGHTS_CUSTOM_DIMS } from '../scripts/lib/ga4-custom-dimensions.mjs';
...
describe('employer insights GA4 custom dimensions (#9403)', () => {
  ...
  for (const dimension of EMPLOYER_INSIGHTS_CUSTOM_DIMS) registered.add(dimension.parameterName);
  ...
  it('finds the event-scoped registrar list', () => {
    ...
    expect(analyticsReport).toContain('...EMPLOYER_INSIGHTS_CUSTOM_DIMS');
  });
