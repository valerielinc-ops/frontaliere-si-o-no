...
describe('committed sitemaps — the #5110 invariant, end to end', () => {
  ...
  it('recovers the alternates the defect removed from sitemap-pages.xml', () => {
    const { entries } = collectLocaleVariantEntries(sources, () => true);
    const before = countAnnotations(readSeeded('sitemap-pages.xml'));
    // 1525, was 1480 when #5110 landed: this branch adds the nine Grigioni/
    // Vallese-Italy crossings to the seeded sitemap (9 × it/en/de/fr/x-default
    // = 45), regenerated by scripts/sync-border-wait-static-sitemap.mjs. The
    // tripwire is kept as an exact count on purpose — the committed sitemap is
    // a static file nothing rebuilds, so an UNEXPLAINED change to it is exactly
    // what this assertion exists to surface.
    expect(before).toBe(1525);
