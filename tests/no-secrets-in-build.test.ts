...
// NOTE: We use structural patterns only — no real keys or key prefixes in test code
// to avoid triggering security scanners (GitHub secret scanning, GitGuardian, etc.)
const SECRET_PATTERNS = [
  // GitHub Personal Access Tokens
  /github_pat_[A-Za-z0-9_]{30,}/,
  // Generic GitHub tokens
  /ghp_[A-Za-z0-9]{36}/,
  // Google API keys (AIzaSy prefix — 39 chars total)
  /AIzaSy[A-Za-z0-9_-]{33}/,
  // reCAPTCHA site keys (6L prefix + 38 chars)
  /6L[a-zA-Z0-9]{8}AAAAA[A-Za-z0-9_-]{17,}/,
...
describe('No secrets in build output', () => {
  ...
  it('should not inject secrets via vite define block', () => {
    ...
    if (defineMatch) {
      ...
      // Allow non-secret defines (e.g. __BUILD_ID__) but forbid secret env vars
      expect(block).not.toMatch(/JSON\.stringify\s*\(\s*process\.env/);
      expect(block).not.toMatch(/JSON\.stringify\s*\(\s*import\.meta\.env/);
