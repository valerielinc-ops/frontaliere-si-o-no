...
describe('assisted application JobBoard handoff', () => {
  ...
  it('uses the original employer destination only after rewarded continue', () => {
    ...
    expect(jobBoardSource).toMatch(
      /const handleRewardedApplicationUnavailable = \(reason: string\) => \{[\s\S]*?trackAssistedApplicationEvent\([\s\S]*?'rewarded_application_inline_unavailable'[\s\S]*?reason[\s\S]*?handoff: 'direct_external'[\s\S]*?redirectExternalApplication\(job, 'rewarded_application_inline_unavailable', true, true\);/,
    );
    expect(jobBoardSource).toMatch(
