...
import { getCantonI18nParams, loadTabTranslations, t } from '../../services/i18n';
...
describe('BlogArticles — contextual CTA canton interpolation (#7308)', () => {
  it('passes canton interpolation params to contextual CTA title and description', () => {
    ...
    expect(CONTEXTUAL_CTA).not.toMatch(/t\(cta\.(titleKey|descKey)\)/);
    expect(CONTEXTUAL_CTA).not.toContain('{canton}');
  });

  it('replaces the localized job-board CTA placeholder with the canton label', async () => {
    await loadTabTranslations('stats');
    const params = getCantonI18nParams('TI');
    const rendered = [
      t('blog.cta.jobBoard.title', params),
      t('blog.cta.jobBoard.desc', params),
    ].join(' ');

    expect(rendered).not.toMatch(/\{canton(?:Prep)?\}/);
    expect(rendered).toContain(params.canton);
  });
});
