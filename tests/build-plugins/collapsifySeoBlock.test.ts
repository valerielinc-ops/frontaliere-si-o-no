...
// color now binds to `var(--color-*)` so light + dark mode work uniformly.
// Background `#f1f5f9` on the card grid is intentionally kept hex for now —
// it's the only non-text color in this fixture and matches the production
// source. The "strips every #rrggbb" assertion in this file allows that one
// because collapsifySeoBlock replaces the card div with `.seo-fb-card` class
// (the hex never reaches the final output).
const SAMPLE_BLOCK = `<aside id="hp-seo-block" style="max-width:1100px;margin:32px auto 56px;padding:0 20px;font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;color:var(--color-body);line-height:1.65" aria-labelledby="hpSeoTitle"><h2 id="hpSeoTitle" style="font-size:24px;font-weight:700;color:var(--color-heading);margin:0 0 14px">Titolo principale</h2><p style="margin:0 0 14px">Paragrafo intro con prosa specifica del frontaliere.</p><h2 style="font-size:20px;font-weight:700;color:var(--color-heading);margin:24px 0 12px">Sezione uno</h2><p style="margin:0 0 14px">Contenuto sezione uno.</p><h2 style="font-size:20px;font-weight:700;color:var(--color-heading);margin:24px 0 12px">Sezione due</h2><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:0 0 22px"><div style="background:#f1f5f9;border-radius:12px;padding:16px"><h3 style="font-size:15px;font-weight:700;margin:0 0 6px;color:var(--color-heading)">Card uno</h3><p style="margin:0;font-size:14px">Descrizione card uno.</p></div></div><h2 style="font-size:20px;font-weight:700;color:var(--color-heading);margin:24px 0 12px">Sezione FAQ</h2><dl style="margin:0 0 22px"><dt style="font-weight:600;color:var(--color-heading);margin-bottom:4px">Domanda?</dt><dd style="margin:0 0 14px">Risposta.</dd></dl><p style="margin:0;font-size:14px;color:var(--color-subtle)">Disclaimer finale.</p></aside>`;
