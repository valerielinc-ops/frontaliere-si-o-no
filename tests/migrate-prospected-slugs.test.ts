  ...
  it('can migrate every checked-in target job in memory without writing cron data', () => {
    ...
    for (const entry of entries) {
      ...
      for (const locale of LOCALES) {
        ...
        const previousSlugs = entry.job.previousSlugsByLocale?.[locale] || [];
        if (oldSlugs[locale] === entry.plan.nextSlugByLocale[locale]) {
          expect(previousSlugs).not.toContain(oldSlugs[locale]);
        } else {
          expect(previousSlugs).toContain(oldSlugs[locale]);
        }
