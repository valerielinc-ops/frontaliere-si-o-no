...
describe('article SEO fallback builder', () => {
  ...
  describe('pipe tables', () => {
    ...
    it('keeps a heading-like table cell inside the table', () => {
      const [section] = cleanupArticleBodySections(keyed([
        [
          '| Voce | Nota con ## marker interno |',
          '|---|---|',
          '| Valore | testo della riga |',
        ].join('\n'),
      ]));

      expect(section.html).toContain('<table>');
      expect(section.html).toContain('<th>Nota con ## marker interno</th>');
      expect(section.html).not.toContain('<h3>');
    });

    it('does not promote a heading inside fenced content', () => {
