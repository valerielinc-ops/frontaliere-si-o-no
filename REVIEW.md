...
Ogni 🟡 nit che sollevi **deve dichiarare la propria disposizione**, così `post-merge-followup` non deve indovinarla e non minta un follow-up di basso valore (il treadmill follow-up brucia ~470 run Claude/sett sulla quota condivisa — vedi `FOLLOWUP.md → Gate grandchild-suppression`):
...
Razionale: la disposizione esplicita sposta il triage del nit **a sinistra** (al review, gratis) invece che **a destra** (minting → `agent:fix` → `pr-review`). Non bloccare l'auto-merge sui nit non-funnel: bloccare ogni nit aggiungerebbe cicli `pr-review-loop` (quota-shift, non quota-saving). Il blocco merge resta solo su 🔴 + process.
...
**Marker `🔴 Important` = stringa esatta, MAI bold.** Scrivi `🔴 Important` (emoji + spazio + parola piana), non `🔴 **Important**` né `🔴 __Important__`. È un marker leggibile da gate deterministici (`pr-redflag-fixer.yml` per auto-fixare il 🔴, `auto-merge-eval.mjs` per bloccare il merge). Un bold rompeva il match literal lasciando la PR bloccata col 🔴 mai indirizzato (PR #2211 round-2). I gate ora sono tolleranti al markdown (regex `🔴\s*\*{0,2}\s*Important`), ma tieni comunque il formato piano: la tolleranza è solo cintura.
