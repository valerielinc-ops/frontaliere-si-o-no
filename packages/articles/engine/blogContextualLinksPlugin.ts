...
export function injectContextualLinks(
  ...
  const maxLinks = opts?.maxLinks ?? shell.contextualLinksMaxPerArticle;
  const rules = opts?.rules ?? shell.contextualLinkRules[locale] ?? [];
  if (rules.length === 0) {
  ...
  for (const rule of rules) {
    ...
    const effectiveMinWords = rule.minArticleWords ?? shell.contextualLinksDefaultMinWords;
