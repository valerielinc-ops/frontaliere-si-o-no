 ...
 *
 * Colocated under packages/articles/engine/shared (issue #4881 Fase 6): the
 * meta-description budget constant lives site-side
 * (`build-plugins/shared/titleSuffix.ts`'s `META_DESCRIPTION_MAX_CHARS`),
 * which this package cannot import directly without breaking confinement —
 * read via `SiteShellContract.metaDescriptionMaxChars` instead (see
 * `../siteShell.ts`).
 */
...
export function boostDescriptionForCtr(
  ...
  const maxLength = opts.maxLength ?? getSiteShell().metaDescriptionMaxChars;
