...
**Dedup a MONTE, non nel triage.** Chiuderli nel triage costerebbe un run per duplicato: si evitano alla sorgente. I monitor usano `scripts/lib/github-issue-creator.mjs`, che con **titolo stabile** (senza run-number) commenta 🔁 sull'issue canonica invece di aprirne una nuova; i follow-up sono **batchati in un solo bucket giornaliero per repository target** da `post-merge-followup` (vedi `FOLLOWUP.md`). La chiave è `follow-up(daily:YYYY-MM-DD)` in `Europe/Zurich`; sito e corpus restano issue separate. Senza duplicati il triage solo classifica+instrada: **puro bash, zero Claude, zero quota**.

### Bucket giornaliero follow-up

Il bucket usa titolo `follow-up(daily:YYYY-MM-DD): N item — owner/repo` e body con
`State: collecting|sealed`, `Daily key`, `Target repository` e item stabili
`FU-YYYY-MM-DD-NNN`. La raccolta può ricevere append da più PR, ma non aggiunge
`agent:fix`/`agent:fix-queued` finché il gate deterministico non ha completato tutti i
chunk, demoto gli item senza acceptance falsificabile e sigillato il body. Il drainer
promuove solo un bucket `sealed`, un item `open` alla volta e senza PR aperta che
dichiari quell'ID; `issue-fix` produce una sola PR per item con `Addresses #N` e
`Follow-up item: FU-...`. Il reconciler marca gli item singolarmente e chiude il
bucket solo quando tutti i validi sono provati `done`. Watermark, retry e marker
`## Post-merge follow-up triage` restano per-run/per-PR e fail-safe.
...
**Pin fuori dal ciclo (`keep-open`, `agent:no-age-out`, #7648).** Una issue che porta una di queste due label è un tracker su una causa che vive FUORI dal repository (review di terzi, decisione altrui, contatore permanente): `classifyIssue` le dà `route='none'` + `autofix=false`, quindi il triage non la instrada, `triage-sweep` la conta marked-only e il drainer disaccoda (senza parcheggiarla: non è lavoro sospeso) quella che fosse già in coda. Prima, `keep-open` vietava solo l'auto-close del reconcile — non la promozione — e una issue che dichiarava nel proprio body di nascere «senza `agent:fix-queued`/`agent:fix`» ha comunque speso un run Max su un'attesa che nessun turn-budget può chiudere. `pinned`/`do-not-close` NON pinnano («non chiudere» ≠ «non riparare») e `revenue`/`tracker` restano instradate (owner decision 2026-07-05). Un `agent:fix` messo **a mano** resta un override valido: il pin agisce sul routing automatico, non sul workflow.
...
Piena regola in AGENTS.md → "Auth automazioni & frugalità quota" (solo `CLAUDE_CODE_OAUTH_TOKEN`, quota condivisa con la sessione owner, triage zero-Claude, dedup a monte, max-turns mai tagliati — cfr. #795/#802/#838). Qui in più: no fixer su issue non-OPEN; concurrency `cancel-in-progress: false` serializza i fixer. **Residuo**: l'auto-route `follow-up` può auto-alimentarsi (fix→merge→followup→bucket giornaliero), bound da bucket condivisi per giorno/repo, **coda drenata un item alla volta** (`followup-drainer`), gate `## LGTM`, **terminazione autonoma** (vedi "Rescue + park"). Un fix pulito non genera nuovi follow-up → converge.
...
**Risolto da `followup-drainer.yml`** (cron ~20min + dispatch manuale, **zero-Claude**, `scripts/ci/followup-drainer.mjs`): ogni categoria ≠ `crawler` entra come `agent:fix-queued`; il drainer ne promuove **UNO** a `agent:fix` solo a slot libero (in-flight==0) → **mai cancellata-in-coda**. Il cron è il trigger automatico durevole: il fan-out `workflow_run` dopo ogni `issue-fix` è stato rimosso perché generava burst concorrenti che GitHub cancellava sul gruppo serializzato. Ordine: `fu-prio:high` prima, poi più vecchia. Usa `isQueueManaged()` (`classifyIssue().route === 'queue'`, condiviso col triage) per rescue/park/age-out. **I crawler, esclusi da `isQueueManaged()`, hanno dal 2026-08-10 un rescue gemello** (`crawlerFixDecision`, #5514): run senza verdetto → ri-arma via coda con `fu-attempt:N` (tetto 3) → `fu-parked` + `needs-human`; `max-turns`/verdetti fermi → park; `rate-limited` → hold/re-queue senza consumare tentativi.
