...
// match feeds a single job pool shared across every locale's nursing page,
// so a mistranslation in any one locale's title must never be able to pull
// an unrelated job in. Every job carries a canonical `job.title` (confirmed
// against the live dataset), so this costs no matching coverage. See #4715
// (same construct fixed in jobSectorLanding.ts::jobMatchesSector).
