...
export function buildTemplateIntro(opts: {
  ...
  if (locale === 'it') {
    ...
    return `In questa pagina trovi ${jobCount} offerte di lavoro per ${keyword}${citySuffix}. Le posizioni sono aggiornate ogni giorno e includono opportunità sia per chi vive in Svizzera sia per i lavoratori frontalieri residenti in Italia. Per ciascun annuncio puoi consultare il salario indicativo (quando disponibile), il tipo di contratto e la sede esatta.${companyPart} Visita le pagine dei singoli annunci per i dettagli completi su mansioni e requisiti.`;
  }
  if (locale === 'en') {
    const companyPart = top3 ? ` Top hiring employers right now: ${top3}.` : '';
    return `This page lists ${jobCount} open positions for ${keyword}${citySuffix}. Listings refresh every day and cover both Swiss residents and Italian cross-border workers. Each ad shows the salary range (when available), contract type, and exact location.${companyPart} Open individual ads for full details on duties and requirements.`;
  }
  if (locale === 'de') {
    const companyPart = top3 ? ` Die wichtigsten Arbeitgeber, die aktuell einstellen: ${top3}.` : '';
    return `Auf dieser Seite finden Sie ${jobCount} offene Stellen für ${keyword}${citySuffix}. Die Inserate werden täglich aktualisiert und decken sowohl in der Schweiz wohnhafte als auch italienische Grenzgängerinnen und Grenzgänger ab. Jedes Inserat zeigt Lohnspanne (sofern verfügbar), Vertragsart und Arbeitsort.${companyPart} Öffnen Sie die einzelnen Inserate für vollständige Angaben zu Aufgaben und Anforderungen.`;
  }
  // fr
  const companyPart = top3 ? ` Les principaux employeurs qui recrutent actuellement : ${top3}.` : '';
  return `Cette page liste ${jobCount} offres d'emploi pour ${keyword}${citySuffix}. Les annonces sont mises à jour chaque jour et concernent aussi bien les résidents suisses que les frontaliers italiens. Chaque annonce indique la fourchette de salaire (lorsqu'elle est disponible), le type de contrat et la localisation exacte.${companyPart} Consultez les annonces individuelles pour le détail des missions et des prérequis.`;
}
