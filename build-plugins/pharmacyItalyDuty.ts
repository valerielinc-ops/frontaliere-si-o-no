...
export function renderItalyDutyWeek({
  ...
  const statusHtml = model.indexable
    ...
    : model.publishable
      ? `<aside style="${BODY_STYLE}"><strong>${esc(copy.partial)}</strong><br>${esc(model.reason)}</aside>`
  ...
  const provinces = model.provinces.map((province) => {
    const rows = province.publishable && province.duties.length > 0
    ...
    const source = province.sourceUrl
      ...
      : '';
    const fetched = province.publishable && province.fetchedAt
...
export function renderItalyDutyCoverageSection(locale: Locale, matrix: DutyCoverageMatrixModel): string {
  ...
  const provinces = matrix.italy.provinces.map((province) => {
    const rows = province.publishable && province.duties.length > 0
      ...
      : `<p style="${BODY_STYLE}">${esc(province.publishable ? copy.noIntervals : copy.italyUnavailableNotice(matrix.italy.state))}</p>`;
    const source = province.sourceUrl
      ? ` <a href="${esc(province.sourceUrl)}" rel="nofollow noopener">${esc(copy.openOfficialSource)}</a>`
      : '';
    return `<section data-coverage-kind="italy-province" data-province-code="${esc(province.code)}"${province.publishable ? ' data-italy-duty-published="true"' : ''}><h3 style="${H3_STYLE}">${esc(province.name)}</h3><p style="${BODY_STYLE}"><strong>${esc(province.publishable ? copy.italyPublishedLabel : copy.italyNotPublishedLabel)}</strong>${source}</p>${rows}</section>`;
  }).join('');
  const notice = matrix.italy.indexable
    ? copy.italyReadyNotice
    : matrix.italy.publishable
      ? `${copy.italyPartialNotice} ${matrix.italy.reason}`
      : `${copy.italyUnavailableNotice(matrix.italy.state)} ${matrix.italy.reason}`;
