  ...
  const PARAS: Record<HubLocale, Record<HubKeyName, [string, string]>> = {
    ...
    en: {
      jobs: [
        ...
        `How to use it as a cross-border worker. To make the most of this list as a frontaliere, combine it with three conceptual filters: distance from your province of residence to the work address (target the smoothest crossings — Brogeda for the Mendrisiotto/Luganese, Stabio for those starting from Varese, Gaggiolo for the southern Mendrisiotto), sector (categories like healthcare, engineering and finance regularly hire 60 %+ cross-border, while public administration and regulated industries enforce stricter residence rules) and salary band (hop to the sector page for min/median/max in your category). The <a class="s-IjpSYt" href="${BASE_URL}/en/calculate-salary/">salary simulator</a> turns gross into net inclusive of the G permit + 2024 Italy-Switzerland agreement.`,
      ],
      sectors: [
...
function buildHubFooterHtml(locale: HubLocale, hubKey: HubKeyName): string {
  ...
  const PARAS: Record<HubLocale, Record<HubKeyName, [string, string]>> = {
    it: {
      ...
      sectors: [
        `Perché l'indice settoriale è il punto di partenza giusto. Per chi cerca lavoro come frontaliere il "filtro per settore" è quasi sempre più utile del filtro per parola chiave: la stessa mansione viene pubblicata con sinonimi diversi (es. "infermiere", "OSS", "personale sanitario") e il classifier testuale che alimenta queste pagine raccoglie tutte le varianti sotto una stessa categoria. Aprire la pagina settoriale dà una panoramica della concorrenza tra datori di lavoro nello stesso ramo, una mediana salariale di riferimento e l'elenco delle aziende che assumono di più — tre informazioni che la sola lista alfabetica delle offerte non offre. Il <a class="s-IjpSYt" href="${BASE_URL}${calcPath}">simulatore stipendio</a> traduce il lordo settoriale in netto reale.`,
...
function buildHtml(args: BuildHtmlArgs): string {
  ...
  const recencyChipsHtml = (hubKey === 'jobs' && locale === 'it')
    ? `<nav class="s-MNH72W" aria-label="Offerte recenti"><span class="s-acyUiE">Offerte recenti:</span><a class="s-vGhlzt" href="/cerca-lavoro-ticino/offerte-da-ieri/">Ultime 24h</a><a class="s-vGhlzt" href="/cerca-lavoro-ticino/offerte-ultimi-3-giorni/">Ultimi 3 giorni</a><a class="s-vGhlzt" href="/cerca-lavoro-ticino/offerte-ultima-settimana/">Ultima settimana</a></nav>`
    : '';
  ...
  <body class="bg-surface-alt text-heading overflow-x-hidden">
    ...
    <main class="seo-static-content s-xzWvwM">
      <nav class="s-AxRVCF" aria-label="Breadcrumb">
        <a class="s-wfUMYx" href="${BASE_URL}/">Home</a>
        <span> / </span>
...
<html lang="${locale}">
  ...
  <body class="bg-surface-alt text-heading overflow-x-hidden">
    ...
    <main class="seo-static-content s-xzWvwM">
      <nav class="s-AxRVCF" aria-label="Breadcrumb">
        <a class="s-wfUMYx" href="${BASE_URL}/">${esc(homeLabel)}</a>
        <span> / </span>
      ...
      ${paginationHtml}
      ${page === 1 ? `<details class="s-2HdGmO">
        <summary class="s--3WWBp">${esc(proseDetailsLabel)} <span class="s-FxywTT" aria-hidden="true"> ▾</span></summary>
        <div class="s-2Hpb5V">
          <p class="s-lUtvMX">${esc(bodyCopy)}</p>
  ...
  // postedAt window. If they earn clicks, generalize to all cantons + locales.
  {
    ...
    for (const r of RECENCY_PAGES) {
      ...
      const patched = html
        ...
        // plugin's hero copy is a one-off override, not a shared template).
        // The replacement re-emits the same fixed style — keep these as
        // literal strings (skipped by the class-extract codemod) so the
        // post-walk patch stays a 1:1 surgical swap.
