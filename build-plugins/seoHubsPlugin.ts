...
function buildHubFooterHtml(locale: HubLocale, hubKey: HubKeyName): string {
  ...
  const PARAS: Record<HubLocale, Record<HubKeyName, [string, string]>> = {
    it: {
      ...
      sectors: [
        `Perché l'indice settoriale è il punto di partenza giusto. Per chi cerca lavoro come frontaliere il "filtro per settore" è quasi sempre più utile del filtro per parola chiave: la stessa mansione viene pubblicata con sinonimi diversi (es. "infermiere", "OSS", "personale sanitario") e il classifier testuale che alimenta queste pagine raccoglie tutte le varianti sotto una stessa categoria. Aprire la pagina settoriale dà una panoramica della concorrenza tra datori di lavoro nello stesso ramo, una mediana salariale di riferimento e l'elenco delle aziende che assumono di più — tre informazioni che la sola lista alfabetica delle offerte non offre. Il <a class="s-IjpSYt" href="${calcPath}">simulatore stipendio</a> traduce il lordo settoriale in netto reale.`,
    ...
    en: {
      jobs: [
        `Why this index exists and who it serves. The alphabetical openings page is the navigational backbone of the job board: cross-border workers find a specific role straight away (e.g. "CNC operator", "home-care nurse"), search engines crawl the archive deeply without depending on JavaScript and feed aggregators get a stable anchor for alert pipelines. The optimal flow when looking for a job as a frontaliere is: open the specific role's page → read the description, location and contract type → compare the gross figure with the net estimated by the <a class="s-IjpSYt" href="${calcPath}">salary calculator</a> (G permit, cantonal withholding, social charges) → factor in commute costs before deciding.`,
...
export function buildThinCantonHubHtml(args: {
  ...
  // Page numbers use the compact, bare-anchor window already used by the
  // master-hub renderer. Applying it to page 1 as well keeps every canton hub
  // page O(1) in pagination markup instead of emitting an O(totalPages) ladder
  // from the most-linked page. The permanent O(√totalPages) depth
  // redistribution is tracked separately in #7803/#4209.
  let paginationHtml = '';
  if (totalPages > 1) {
    ...
    // Bare page number (was `${pageWord}&nbsp;${p}`). Every page uses the
    // first, last and adjacent pages. The `<details><summary>` +
    // `<nav aria-label>` retain the browse context and the prev/next chain
    // remains explicit.
