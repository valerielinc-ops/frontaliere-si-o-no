...
type FuelPricesExchangeSnapshot = {
 readonly generatedAt?: string;
 readonly sources?: {
  readonly exchangeRate?: {
   readonly eurPerChf?: number;
  };
 };
};

const COMPANY_CONTEXT_FX_FALLBACK_RATE = 1.08;

function snapshotFxRate(): number {
 const rate = (fuelPricesSnapshot as FuelPricesExchangeSnapshot).sources?.exchangeRate?.eurPerChf;
 return typeof rate === 'number' && Number.isFinite(rate) && rate > 0 ? rate : COMPANY_CONTEXT_FX_FALLBACK_RATE;
}

function snapshotFxDate(locale: CompanyHubFrontalierContextLocale): string {
 const generatedAt = (fuelPricesSnapshot as FuelPricesExchangeSnapshot).generatedAt;
 const date = generatedAt ? new Date(generatedAt) : null;
 if (!date || Number.isNaN(date.getTime())) {
  return locale === 'it' ? 'snapshot locale' : 'local snapshot';
 }
...
export function renderCompanyHubFrontalierContext(
 ...
 // Build-time static HTML uses the local exchange-rate snapshot also used by
 // fuel-prices pages; the SPA calculator refreshes the same concept at runtime.
 const grossMid = bands.mid[0];
 const netRatio = 0.78;
 const fxRate = snapshotFxRate();
 const fxRateLabel = locale === 'en' ? fxRate.toFixed(2) : fxRate.toFixed(2).replace('.', ',');
 const fxDateLabel = snapshotFxDate(locale);
