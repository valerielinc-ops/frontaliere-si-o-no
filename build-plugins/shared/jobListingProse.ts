  ...
  it: [
    {
      ...
      crossLinks: `Prima di candidarti consigliamo due passaggi rapidi: simula lo stipendio netto con il <a class="s-IjpSYt" href="${CALC_HREF.it}">${CALC_LABEL.it}</a> per capire se l\'offerta ha senso rispetto al tuo lavoro attuale, e verifica il <a class="s-IjpSYt" href="${FX_HREF.it}">${FX_LABEL.it}</a> per impostare il bonifico CHF→EUR al cambio più conveniente. Se l\'azienda offre opzioni LAMal, confronta i premi nel <a class="s-IjpSYt" href="${HEALTH_HREF.it}">${HEALTH_LABEL.it}</a>.`,
      faq: [
        {
    ...
    },
    {
      ...
      crossLinks: `Per pianificare il budget mensile reale combina il <a class="s-IjpSYt" href="${CALC_HREF.it}">${CALC_LABEL.it}</a> con il <a class="s-IjpSYt" href="${FX_HREF.it}">${FX_LABEL.it}</a>: la differenza tra cambio interbancario e cambio retail di una banca italiana può costare fino a 2 % all\'anno sullo stipendio. Per la scelta cassa malati, il nostro <a class="s-IjpSYt" href="${HEALTH_HREF.it}">${HEALTH_LABEL.it}</a> mostra i premi LAMal per comune ticinese.`,
      faq: [
        {
    ...
    },
    {
      ...
      crossLinks: `Il <a class="s-IjpSYt" href="${CALC_HREF.it}">${CALC_LABEL.it}</a> implementa entrambi i regimi (vecchio + nuovo accordo) e mostra il rimborso ricevuto dal proprio comune. Per chi pendola da zona Como/Varese, il <a class="s-IjpSYt" href="${FX_HREF.it}">${FX_LABEL.it}</a> confronta i tassi di cambio applicati da banche italiane vs cambia-valute svizzeri vs Wise/Revolut. Il <a class="s-IjpSYt" href="${HEALTH_HREF.it}">${HEALTH_LABEL.it}</a> spiega quando conviene optare per LAMal vs SSN italiano.`,
      faq: [
        {
  ...
  en: [
    {
      ...
      crossLinks: `Two quick steps before applying: simulate net pay with the <a class="s-IjpSYt" href="${CALC_HREF.en}">${CALC_LABEL.en}</a> to see whether the listing improves your current package, and check the <a class="s-IjpSYt" href="${FX_HREF.en}">${FX_LABEL.en}</a> to time CHF→EUR transfers at the best rate. If the employer offers LAMal coverage, compare premiums in the <a class="s-IjpSYt" href="${HEALTH_HREF.en}">${HEALTH_LABEL.en}</a>.`,
      faq: [
        {
    ...
    },
    {
      ...
      crossLinks: `To plan a real monthly budget combine the <a class="s-IjpSYt" href="${CALC_HREF.en}">${CALC_LABEL.en}</a> with the <a class="s-IjpSYt" href="${FX_HREF.en}">${FX_LABEL.en}</a>: the spread between interbank and Italian-bank retail FX can cost up to 2 % per year on your salary. For health insurance, our <a class="s-IjpSYt" href="${HEALTH_HREF.en}">${HEALTH_LABEL.en}</a> shows LAMal premiums by Ticino municipality.`,
      faq: [
        {
    ...
    },
    {
      ...
      crossLinks: `The <a class="s-IjpSYt" href="${CALC_HREF.en}">${CALC_LABEL.en}</a> implements both regimes (old + new agreement) and shows the refund returned by your municipality. For commuters from Como or Varese, the <a class="s-IjpSYt" href="${FX_HREF.en}">${FX_LABEL.en}</a> compares rates from Italian banks vs Swiss bureaus de change vs Wise/Revolut. The <a class="s-IjpSYt" href="${HEALTH_HREF.en}">${HEALTH_LABEL.en}</a> covers when LAMal beats the Italian SSN.`,
      faq: [
        {
  ...
  de: [
    {
      ...
      crossLinks: `Zwei schnelle Schritte vor der Bewerbung: Netto-Lohn mit dem <a class="s-IjpSYt" href="${CALC_HREF.de}">${CALC_LABEL.de}</a> simulieren, um zu sehen, ob das Angebot Ihren aktuellen Gehalt übertrifft, und den <a class="s-IjpSYt" href="${FX_HREF.de}">${FX_LABEL.de}</a> prüfen, um CHF→EUR-Überweisungen zum besten Tarif zu planen. Wenn der Arbeitgeber LAMal anbietet, die Prämien im <a class="s-IjpSYt" href="${HEALTH_HREF.de}">${HEALTH_LABEL.de}</a> vergleichen.`,
      faq: [
        {
    ...
    },
    {
      ...
      crossLinks: `Für ein realistisches Monatsbudget kombinieren Sie den <a class="s-IjpSYt" href="${CALC_HREF.de}">${CALC_LABEL.de}</a> mit dem <a class="s-IjpSYt" href="${FX_HREF.de}">${FX_LABEL.de}</a>: die Differenz zwischen Interbankenkurs und italienischem Bankenkurs kostet bis zu 2 % pro Jahr auf das Gehalt. Zur Krankenkassenwahl zeigt unser <a class="s-IjpSYt" href="${HEALTH_HREF.de}">${HEALTH_LABEL.de}</a> die LAMal-Prämien nach Tessiner Gemeinde.`,
      faq: [
        {
    ...
    },
    {
      ...
      crossLinks: `Der <a class="s-IjpSYt" href="${CALC_HREF.de}">${CALC_LABEL.de}</a> implementiert beide Regime (altes + neues Abkommen) und zeigt die Rückerstattung Ihrer Gemeinde. Für Pendler aus dem Raum Como/Varese vergleicht der <a class="s-IjpSYt" href="${FX_HREF.de}">${FX_LABEL.de}</a> die Kurse italienischer Banken vs Schweizer Wechselstuben vs Wise/Revolut. Der <a class="s-IjpSYt" href="${HEALTH_HREF.de}">${HEALTH_LABEL.de}</a> beschreibt, wann sich LAMal gegenüber dem italienischen SSN lohnt.`,
      faq: [
        {
  ...
  fr: [
    {
      ...
      crossLinks: `Deux étapes rapides avant de postuler : simuler le net avec le <a class="s-IjpSYt" href="${CALC_HREF.fr}">${CALC_LABEL.fr}</a> pour vérifier si le poste améliore votre situation, et consulter le <a class="s-IjpSYt" href="${FX_HREF.fr}">${FX_LABEL.fr}</a> pour planifier les transferts CHF→EUR au meilleur taux. Si l\'employeur propose une couverture LAMal, comparez les primes dans le <a class="s-IjpSYt" href="${HEALTH_HREF.fr}">${HEALTH_LABEL.fr}</a>.`,
      faq: [
        {
    ...
    },
    {
      ...
      crossLinks: `Pour planifier un budget mensuel réaliste, combinez le <a class="s-IjpSYt" href="${CALC_HREF.fr}">${CALC_LABEL.fr}</a> avec le <a class="s-IjpSYt" href="${FX_HREF.fr}">${FX_LABEL.fr}</a> : l\'écart entre taux interbancaire et taux retail des banques italiennes peut coûter jusqu\'à 2 % par an sur le salaire. Pour le choix de la caisse maladie, notre <a class="s-IjpSYt" href="${HEALTH_HREF.fr}">${HEALTH_LABEL.fr}</a> affiche les primes LAMal par commune tessinoise.`,
      faq: [
        {
    ...
    },
    {
      ...
      crossLinks: `Le <a class="s-IjpSYt" href="${CALC_HREF.fr}">${CALC_LABEL.fr}</a> implémente les deux régimes (ancien + nouvel accord) et affiche le remboursement reversé par votre commune. Pour les frontaliers de Côme/Varèse, le <a class="s-IjpSYt" href="${FX_HREF.fr}">${FX_LABEL.fr}</a> compare les taux des banques italiennes vs bureaux de change suisses vs Wise/Revolut. Le <a class="s-IjpSYt" href="${HEALTH_HREF.fr}">${HEALTH_LABEL.fr}</a> explique quand LAMal bat le SSN italien.`,
      faq: [
        {
...
export function renderListingPaginationProse(
  ...
  const faqHtml = v.faq
    .map(
      (f) =>
        `<details class="s-TdgkK3"><summary class="s-HBR0NM">${escAttr(f.question)}</summary><p class="s-bOIp6r">${escAttr(f.answer)}</p></details>`,
    )
    .join('');

  return `<section class="s-A_RnbE">
  <h2 class="s-jEiAXZ">${escAttr(labels.sectionHeading)}</h2>
  <p class="s-clIDbe">${v.methodology}</p>
  <p class="s-clIDbe">${v.context}</p>
  <p class="s-clIDbe">${v.crossLinks}</p>
  <p class="s--0PHnG"><strong class="s-MLbk8r">${escAttr(locale === 'it' ? 'Esempio' : locale === 'en' ? 'Example' : locale === 'de' ? 'Beispiel' : 'Exemple')}:</strong> ${v.scenario}</p>
  <h3 class="s-HoWkrF">${escAttr(labels.faqHeading)}</h3>
  ${faqHtml}
</section>`;
}
