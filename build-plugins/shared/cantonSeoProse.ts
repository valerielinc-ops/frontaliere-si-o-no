...
/**
 * Format a CHF integer with the thousands separator used in each locale's
 * prose: apostrophe for it/de ("85'000"), comma for en ("85,000"), space for
 * fr ("85 000").
 */
function fmtChf(n: number, locale: CantonSeoLocale): string {
...
function buildDeepDiveBlock(opts: CantonSeoProseOpts): string {
  ...
  // Net-pay scenario paragraph — the gross + net figures are scaled to the
  // canton's official BFS wage level (cantonGrossSalaryBand) rather than a
  // single hard-coded national bracket, so a Zürich page shows higher numbers
  // than a Ticino or Ostschweiz page. Unmapped cantons fall back to the
  // national-average band, so the numbers are realistic without overclaiming.
  const g = cantonGrossSalaryBand(canton);
  const grossRange = `CHF ${fmtChf(g.grossLow, locale)}-${fmtChf(g.grossHigh, locale)}`;
  const netpayP = (() => {
    if (locale === 'it') {
      return `Per un ruolo professionale tipico nel Canton ${canton} (lordo annuo ${grossRange}, 13 mensilità incluse, AVS-AI-IPG 5,3 %, AD 1,1 %, LPP 7-15 % in funzione dell'età, imposta alla fonte cantonale 4-14 % a seconda dello scaglione e stato civile), il netto mensile per un single senza figli è di norma fra CHF ${fmtChf(g.netSingleLow, locale)} e CHF ${fmtChf(g.netSingleHigh, locale)}; per una coppia con due figli a carico, grazie alle deduzioni familiari cantonali, fra CHF ${fmtChf(g.netCoupleLow, locale)} e CHF ${fmtChf(g.netCoupleHigh, locale)}. Con il Nuovo Accordo Italia-Svizzera 2024 (in vigore per i frontalieri assunti dopo il 17 luglio 2023) si applica la tassazione concorrente: il datore svizzero trattiene l'80 % della ritenuta cantonale ordinaria, il contribuente dichiara il reddito in Italia con franchigia di EUR 10'000 e credito d'imposta per la parte trattenuta in Svizzera. Per chi era già frontaliere prima del 17 luglio 2023, il vecchio regime resta in vigore (tassazione esclusivamente svizzera, ristorno del 38,8 % al comune italiano di residenza). Apri il <a href="${CALCULATOR_HREF[locale]}">calcolatore stipendio Frontaliere Ticino</a> e inserisci il lordo dell'annuncio che ti interessa nel Canton ${canton}: ottieni il netto mensile in CHF e in EUR, comparabile con un'offerta italiana, in meno di 30 secondi.`;
    }
    if (locale === 'en') {
      return `For a typical professional role in Canton ${canton} (annual gross ${grossRange} inclusive of 13 monthly payments, AVS-AI-IPG 5.3 %, unemployment 1.1 %, LPP 7-15 % by age band, cantonal source tax 4-14 % depending on bracket and civil status), the monthly net for a single applicant with no children typically runs between CHF ${fmtChf(g.netSingleLow, locale)} and CHF ${fmtChf(g.netSingleHigh, locale)}; for a married couple with two dependent children, after cantonal family deductions, between CHF ${fmtChf(g.netCoupleLow, locale)} and CHF ${fmtChf(g.netCoupleHigh, locale)}. Under the 2024 Italy-Switzerland bilateral agreement (in force for cross-border workers hired after 17 July 2023) dual taxation applies: the Swiss employer withholds 80 % of the standard cantonal rate, the taxpayer declares the income in Italy with a 10,000 EUR allowance and tax credit for the Swiss withholding. Cross-border workers already classified before 17 July 2023 keep the old regime (exclusive Swiss taxation, 38.8 % refund to the Italian residence municipality). Open the <a href="${CALCULATOR_HREF[locale]}">Frontaliere Ticino salary calculator</a> with the gross figure from a Canton ${canton} listing: you'll get the monthly net in CHF and EUR, directly comparable with an Italian offer, in under 30 seconds.`;
    }
    if (locale === 'de') {
      return `Für eine typische Fachrolle im Kanton ${canton} (Bruttojahreslohn ${grossRange} inkl. 13. Monatslohn, AHV-IV-EO 5,3 %, ALV 1,1 %, BVG 7-15 % nach Altersband, kantonale Quellensteuer 4-14 % je nach Stufe und Zivilstand) liegt das Monatsnetto für Alleinstehende ohne Kinder typischerweise zwischen CHF ${fmtChf(g.netSingleLow, locale)} und CHF ${fmtChf(g.netSingleHigh, locale)}; für ein Ehepaar mit zwei Kindern nach kantonalen Familienabzügen zwischen CHF ${fmtChf(g.netCoupleLow, locale)} und CHF ${fmtChf(g.netCoupleHigh, locale)}. Mit dem Steuerabkommen Italien-Schweiz 2024 (gültig für Grenzgänger, die nach dem 17. Juli 2023 angestellt wurden) gilt die konkurrierende Besteuerung: Der Schweizer Arbeitgeber zieht 80 % des ordentlichen kantonalen Satzes ab, der Steuerpflichtige deklariert das Einkommen in Italien mit Freibetrag von EUR 10'000 und Steuergutschrift für den in der Schweiz einbehaltenen Anteil. Vor dem 17. Juli 2023 bereits eingestufte Grenzgänger behalten das alte Regime (ausschliesslich Schweizer Besteuerung, 38,8 % Rückerstattung an die italienische Wohngemeinde). Öffnen Sie den <a href="${CALCULATOR_HREF[locale]}">Frontaliere-Ticino-Lohnrechner</a> mit dem Bruttowert eines Inserats aus dem Kanton ${canton}: in unter 30 Sekunden erhalten Sie das Monatsnetto in CHF und EUR, direkt vergleichbar mit einem italienischen Angebot.`;
    }
    return `Pour un poste professionnel typique dans le canton ${canton} (brut annuel ${grossRange} incluant le 13e mois, AVS-AI-APG 5,3 %, chômage 1,1 %, LPP 7-15 % selon la tranche d'âge, impôt à la source cantonal 4-14 % selon la tranche et l'état civil), le net mensuel pour un célibataire sans enfants se situe généralement entre CHF ${fmtChf(g.netSingleLow, locale)} et CHF ${fmtChf(g.netSingleHigh, locale)} ; pour un couple marié avec deux enfants à charge, après déductions familiales cantonales, entre CHF ${fmtChf(g.netCoupleLow, locale)} et CHF ${fmtChf(g.netCoupleHigh, locale)}. Avec l'accord fiscal Italie-Suisse 2024 (en vigueur pour les frontaliers engagés après le 17 juillet 2023), la taxation concurrente s'applique : l'employeur suisse retient 80 % du taux cantonal ordinaire, le contribuable déclare le revenu en Italie avec abattement de EUR 10 000 et crédit d'impôt pour la retenue suisse. Les frontaliers déjà classés avant le 17 juillet 2023 conservent l'ancien régime (taxation exclusivement suisse, rétrocession de 38,8 % à la commune italienne de résidence). Ouvrez le <a href="${CALCULATOR_HREF[locale]}">calculateur de salaire Frontaliere Ticino</a> avec le brut d'une annonce du canton ${canton} : en moins de 30 secondes vous obtenez le net mensuel en CHF et en EUR, directement comparable à une offre italienne.`;
  })();
