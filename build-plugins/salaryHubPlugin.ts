...
export function salaryHubPlugin(rootDir: string): Plugin {
  return {
    ...
    async closeBundle() {
      ...
      // function of the (deterministic) sitemapEntries.
      ...
      // ── Patch sitemap.xml index with today's date ────────────────
      // The sitemap.xml index file is re-emitted by other plugins each
      // build, so our entry's <lastmod> would otherwise drop out. Date
      // is intentionally today (the index advertises "this submap was
      // last touched on date X").
