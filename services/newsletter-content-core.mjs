 ...
 *
 * PRIVO DI DIPENDENZE NODE — deliberatamente (#8125). `services/newsletterPreview.ts`
 * importa da qui per la preview dell'AdminPanel, quindi questo modulo sta nel
 * grafo del bundle browser: rollup risolve un builtin Node a
 * `__vite-browser-external`, che non ha export nominati, e il link della build
 * muore — dopo ~80 minuti per locale. Il try/catch attorno alla lettura da disco
 * non proteggeva da niente, perche' il fallimento e' della RISOLUZIONE del
 * binding, non dell'esecuzione: bastava riscrivere in forma nominale l'import
 * di default del builtin.
 *
 * I dataset su disco arrivano dal reader iniettato in `./newsletter-datasets.mjs`,
 * che `./newsletter-content.mjs` installa. I consumatori Node importano
 * QUEL modulo — ri-esporta tutto quanto sta qui — e leggono gli stessi file di
 * prima; il browser resta senza reader e ricade sui fallback gia' previsti.
 */
...
function loadLogoManifest() {
  ...
  _logoManifest = readNewsletterDataset('data', 'company-logos-manifest.json') || {};
...
function loadPopularity() {
  ...
  const data = readNewsletterDataset('data', 'job-popularity.json');
...
export function loadDashboardMetrics() {
  ...
  // ── Unemployment rate ──
  try {
    const unempData = readNewsletterDataset('public', 'data', 'switzerland-unemployment-rate.json');
  ...
  try {
    ...
    const candidates = [
      ['data', 'health-premiums', `${currentYear}.json`],
      ['data', 'health-premiums.json'],
    ];
    let lamalData = null;
    for (const segments of candidates) {
      lamalData = readNewsletterDataset(...segments);
      if (lamalData) break;
    }
...
function loadCantonResolvers() {
  ...
  try {
    const cantonSlugFile = readNewsletterDataset('data', 'canton-url-slugs.json');
    const municipalitiesFile = readNewsletterDataset('data', 'canton-municipalities.json');
    if (!cantonSlugFile || !municipalitiesFile) throw new Error('canton datasets unavailable');
