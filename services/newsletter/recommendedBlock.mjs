/**
 * recommendedBlock.mjs — config-driven "Consigliato per te" revenue block for
 * every email we send (weekly newsletter + onboarding drip). Issue #4450.
 *
 ...
 *      services/affiliatePartnersRegistry.mjs (single source of the enable
 *      gate; a partner disabled there disappears from email too, no drift).
 *      Each email click routes through the /go/{id}/ redirect page for uniform
 *      edge attribution, exactly like on-site partner cards.
 *
 ...
 * from services/affiliatePartnersRegistry.mjs at selection time). An entry
 * whose partner is disabled there is skipped automatically.
 *
