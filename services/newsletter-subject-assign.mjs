 ...
 * only by server scripts (send-newsletter, newsletter-ab-report via
 * lib/newsletter-ab-data) and tests — keeps `node:crypto` out of the SPA graph.
 *
 * The hash function is BYTE-IDENTICAL to the previous in-line implementation, so
 * variant assignment is unchanged: every (email, campaignId[, opts]) maps to the
 * same variant as before, and the A/B report can still re-compute historical
...
import {
  ...
  DEFAULT_EPSILON,
  normalizeEmail,
} from './newsletter-subject-variants.mjs';
