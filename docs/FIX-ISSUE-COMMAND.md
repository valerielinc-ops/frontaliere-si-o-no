...
- **Claim mutex** (#4788/#4793 — una sessione locale e `issue-fix.yml` hanno fixato la stessa issue in parallelo, due PR competitive in conflitto al merge): reclama il lock SUBITO, prima di ogni altro passo, con `CLAIM_OWNER=local CLAIM_ACTION=acquire ISSUE_NUMBER=$ARGUMENTS node scripts/ci/claim-issue-in-flight.mjs`. Il comando aggiunge `agent:in-progress` + `agent:local` e verifica il claim; se stampa `in_flight=true` o `claim_error=true`, un'altra sessione (locale o il fixer CI) la sta già lavorando oppure il lock non è leggibile → fermati, avvisa l'utente, NON procedere. Non usare più `gh issue edit --add-label agent:in-progress` da solo: non identifica il proprietario e il fixer remoto potrebbe rimuovere il lock locale.
...
- **Stati già dichiarati nei residui** (stessa regola di `issue-fix.yml` e `pr-redflag-fixer.yml` — AGENTS.md #6: la superficie locale non può contraddire quelle CI). Se la issue nasce da una PR, leggi `## Non implementato (ancora)` di quel body **riusando** la tassonomia, mai una regex tua:
  ...
  **Cinque stati CHIUDONO la voce** (`in questa PR`, `PR concatenata #N`, `per scelta`, `by construction`, `blocked: decisione del proprietario` — i `CLOSING_STATES` di `scripts/ci/followup-has-candidates.mjs`), mentre `blocked: <causa tecnica>` resta aperto. `PR concatenata` senza `#N` non è uno stato chiudente e viola il contratto. I primi due stati indicano lavoro già tracciato; gli ultimi tre chiudono solo **col motivo scritto dopo lo stato**. Se il bullet che copre il tema della issue è chiuso, non implementarlo: commenta citando il bullet verbatim e fermati. Prima di onorare il veto controlla che lo stato non stia dentro una negazione («non per scelta ma…»): `bulletState` vince sull'occorrenza più a sinistra e classificherebbe quel bullet come chiuso.
...
7. Push + `gh pr create`. Body `## Implementato` + `## Non implementato (ancora)`. Dopo la creazione, marca la provenienza per abilitare il fixer automatico sui branch locali: `gh label create agent:autofix --color 1d76db --description "PR prodotta dal loop autonomo issue-fix" --force 2>/dev/null || true` e `gh pr edit <PR_NUMBER> --add-label agent:autofix`.
...
9. **Rilascia il claim** (a lavoro concluso o se abbandoni prima): `CLAIM_OWNER=local CLAIM_ACTION=release ISSUE_NUMBER=$ARGUMENTS node scripts/ci/claim-issue-in-flight.mjs`. Il detector stale protegge intenzionalmente `agent:local`, perché non può osservare l'heartbeat della sessione: se la sessione muore, verifica prima a mano e poi usa questo comando. Mai rimuovere il solo label base con `gh issue edit`.
