...
9. **Hospital wave 3 runners** (gzo-wetzikon, spital-maennedorf, spital-uster, ksb, see-spital) — parser scaffolded ma `scripts/update-{slug}-jobs.mjs` skippati. Quick fix: `node scripts/scaffold-crawler.mjs --finish-existing-parser {slug}` (~5 min ognuno) — post-2026-07 consolidation, questo NON genera più un `.github/workflows/update-jobs-{slug}.yml` dedicato: appende a `data/crawler-manifest.json` e rigenera i 23 `crawler-group-*.yml` in automatico.
