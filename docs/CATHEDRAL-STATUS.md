...
9. **Hospital wave 3 runners + workflow manifest entries** (gzo-wetzikon, spital-maennedorf, spital-uster, ksb, see-spital) — parser scaffolded ma `scripts/update-{slug}-jobs.mjs` + `data/crawler-manifest.json` entry (post-consolidation 2026-07: no more standalone `.github/workflows/update-jobs-{slug}.yml`, see `docs/CRAWLERS.md`) skippati. Quick fix: `node scripts/scaffold-crawler.mjs --finish-existing-parser {slug}` poi `node scripts/generate-crawler-group-workflows.mjs` (~5 min ognuno).
