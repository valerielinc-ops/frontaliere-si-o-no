...
8. ~~**Tier-3 marquee Playwright DOM logic** (Richemont, MSC Cargo, Bobst, Vaudoise) — placeholder con `--playwright` flag ma DOM selectors non implementati.~~ ✅ **DONE 2026-05-10** — 2 branches (PR #66 + PR #67):
    ...
    - **Richemont** ⛔ BLOCKED-WAF 2026-05-11 — Cloudflare 403 sulla pagina careers. PR #66 (Adzuna free-tier fallback) revertito: la policy del progetto è "crawl only from origin, no middleware/aggregators" — non ToS-compliant emettere `applyUrl` puntato a un aggregatore di terze parti. Direct crawler ripristinato; resta a zero jobs finché non si risolve l'anti-bot Cloudflare via Playwright stealth o residential proxy. Nessun budget esterno.
    - **MSC Cargo** ⛔ BLOCKED-WAF 2026-05-11 — Akamai BMP 403 sulla pagina careers; stesso pattern Richemont. Direct crawler ripristinato. Nessun middleware Adzuna/aggregator; zero jobs finché origin non è raggiungibile.
