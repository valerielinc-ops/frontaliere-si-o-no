...
```bash
git worktree add ~/Projects/cathedral-finalize -b cathedral-finalize-deploy
cd ~/Projects/cathedral-finalize
```

- [ ] **Step 2: Copy gitignored data files**

```bash
cp ~/Projects/frontaliere-si-o-no/data/jobs.json data/jobs.json
...
```bash
bash ~/Projects/frontaliere-si-o-no/scripts/cathedral-live-smoke.sh
```
...
```bash
cd ~/Projects/frontaliere-si-o-no
git worktree remove --force ~/Projects/cathedral-finalize
