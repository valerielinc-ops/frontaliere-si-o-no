...
- **Extracting/Splitting**: MUST run `gitnexus_context({name: "target"})` for incoming/outgoing refs, then `gitnexus_impact({target: "target", direction: "upstream"})` for external callers before moving code.
...
After committing code changes, the GitNexus index becomes stale:
...
Inspect `.gitnexus/meta.json` (`stats.embeddings` field) to check whether embeddings exist (0 = none). **Running analyze without `--embeddings` will delete any previously generated embeddings.**
