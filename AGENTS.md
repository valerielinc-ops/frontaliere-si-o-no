...
8. **Definizione di «fatto» = completezza totale, non il merge della prima PR.** Chiudi un task solo quando `## Non implementato (ancora)` è «Nessuno» e tutto lo scope è live. Il deferral mantiene il task aperto: continua nella stessa PR o in PR concatenate. «Out of scope» vale solo per un task davvero diverso e va separato. Gli stati chiudenti sono quelli di `CLOSING_STATES`: `in questa PR`, `PR concatenata #N`, `blocked: decisione del proprietario`, `per scelta`, `by construction`; gli ultimi due richiedono il motivo esplicito. `blocked: <causa tecnica>` resta aperto mentre si tenta di rimuovere il blocco (vedi `## Build And Test → Blocker ≠ stallo`). Reviewer: residuo senza piano o stato senza motivo = 🔴 (vedi `REVIEW.md`).
