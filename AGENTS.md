...
9. **Esperimenti futuri: solo Firebase, mai PostHog.** Ogni nuovo A/B test, feature flag sperimentale o confronto di varianti deve usare Firebase (Remote Config per assegnazione/configurazione e Firebase Analytics per la misurazione, quando pertinente). Non introdurre nuovi flag, SDK, eventi o dipendenze PostHog per esperimenti. Il codice e la documentazione PostHog già esistenti sono storico: non vanno estesi senza una decisione esplicita del proprietario.
...
- **Leak locale worktree/branch:** pre-task-close DEVE eseguire `node scripts/prune-merged-worktrees.mjs` in dry-run e poi `--apply`; gli hook `.claude/settings.json` fanno `SessionEnd --apply --orphans-only` e `SessionStart --apply`. Un `fix/issue-N` chiuso senza PR si rimuove solo con `ahead=0`; con commit unici o ahead sconosciuto resta report-only. Il purge riconosce anche snapshot locali tramite commit→PR, richiede una PR `MERGED` verso `main` e crea `snapshot/purge/*` prima di cancellare un branch merged non già taggato. Lo script tocca SOLO `.claude/worktrees`/`.worktrees`/`.wt`; `worktree-branch-janitor.yml` copre il remoto con la stessa guardia `ahead=0`. Dettaglio: `docs/AGENTS-HISTORY.md#worktree-branch-leak`.
...
- **Auth Codex Luna Max = SOLO `CODEX_AUTH_JSON`** (subscription, zero costo API) per i workflow agentici `tests`, `issue-fix` e `post-merge-followup`; il wrapper locale `claude-codex-fallback` inoltra la sessione al broker Codex, ma questi percorsi usano Codex Luna Max e non un provider Claude/Anthropic di fallback. **Mai aggiungere `ANTHROPIC_API_KEY` o `CLAUDE_CODE_OAUTH_TOKEN`** ai workflow agentici.
  - **Eccezione scoped (owner-approved 2026-07-28, issue #4495; Haiku → Codex il 2026-09-24)**: `functions/src/codexFallback.js` è l'ultimo rung delle fallback chain di `geminiGenerate.js`/`chatbotInference.js`, DOPO Gemini e gli OpenAI-compatible free (Groq/NVIDIA). Niente chiave API: usa lo stesso login ChatGPT della CI, dal parametro Remote Config `CODEX_AUTH_JSON`, e chiama il backend Codex come `codex exec` 0.153.4 (`chatgpt.com/backend-api/codex/responses`, SSE; modello `CODEX_FALLBACK_MODEL`, default `gpt-5.6-luna`). **La function non rinfresca mai il login** (refresh token monouso condiviso con la CI): access token scaduto → rung saltato (`codex: auth_expired`). La freschezza viene solo da `codex-auth-rotate.yml`, che dopo ogni refresh validato scrive in Remote Config la copia senza `refresh_token` (ETag). `CODEX_AUTH_JSON` resta fuori da `RC_TO_ENV`: in CI vale solo il secret GitHub.
...
- **Il routing abilita ogni categoria** (DECISIONS.md 2026-09-24, «Nessun veto sul ciclo autonomo»; policy `f1-f7-v4`). Domini F1/F7, control-plane, path e categorie ignoti e `needs-human` sono evidenza, su issue e PR, e non fermano il routing; la supervisione è la PR (test, review, `## LGTM`, HEAD esatta). Nessuna domanda al proprietario: le scelte le prende l'agente con la direttiva «non perdere pagine SEO, revenue o utenti», documentandole nella PR. `crawler` → `agent:fix` diretto (uno per run/slot libero, eccedenza in coda); tutto il resto → `agent:fix-queued`.
- **Claim mutex `agent:in-progress`** (#4788/#4793): dopo la risk policy, quota/preflight, `issue-fix.yml` reclama il claim prima di tier e provider; la concorrenza per issue è `cancel-in-progress: false` e il drainer mantiene un pool bounded. Chi arriva prima vince, l'altro salta senza spendere turni; errori GitHub/API/parse sono fail-closed. Il release remoto è gated su `claim_acquired=true` e rimuove il mutex `agent:in-progress` insieme a `agent:remote` solo quando quel run ne è il proprietario; il detector stale recupera claim remoti/legacy senza PR dopo 12h e claim `agent:local` senza PR dopo 24h dal claim (non dall'ultima attività dei bot).
...
- **Handoff triage→fix richiede App token o PAT**: il triage usa l'installation token App come identità primaria e `GITHUB_PAT` come fallback; una label `agent:fix` via `GITHUB_TOKEN` NON triggera `issue-fix` (anti-ricorsione GitHub) e ha sender `github-actions[bot]` che non passa il gate. `GITHUB_TOKEN` resta per label anti-loop (`agent:triaged`) e per il defer tecnico `automation-deferred`; `needs-human` non si assegna più (il pre-pass lo rimuove e rimette la issue in coda). I workflow che devono triggerarne altri caricano il PAT via `scripts/load-rc-env.mjs` (serve Firebase SA).
