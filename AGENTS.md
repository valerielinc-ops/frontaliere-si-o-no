...
- Leve frugalità attive: triage `--max-turns 8` sonnet; concurrency serializzata (`cancel-in-progress: false`); dedup-storm chiude duplicati prima del routing. **Mai** abbassare max-turns di `pr-review-loop`/`issue-fix`: sono quality gate (Non-Negotiable #1) e turni bassi causano `error_max_turns` senza review (PR #838).
