...
- **Attesa PR = osservazione event-driven.** Dalla root workspace (`~/Projects/frontaliere`), registra una subscription con `bin/gh-frontaliere events subscribe --repo valerielinc-ops/frontaliere-si-o-no --resource pull_request --number <N> --wait-for merged,closed,failed,commented,needs_review,approved --agent-id <id>` (i tre stati di review svegliano sul 🔴: con il solo `merged,failed` una review non sveglia nessuno) e avvia un solo `bin/gh-frontaliere events listen <subscription-id>`; non usare polling di `gh pr view`, `gh run view` o `gh pr checks`. Se la subscription scade, tratta l'esito come `timed-out` e dichiara la prossima azione; a `MERGED` verifica il risultato prima del cleanup. Dettaglio: `docs/AGENTS-HISTORY.md#active-pr-watch`.
