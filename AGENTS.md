...
- PR ready per `main` → reviewer Claude posta review e `auto-merge-on-lgtm.yml` mergia automaticamente se il review body contiene la stringa esatta `## LGTM`. **NIENTE merge manuale, mai:** aspetta che `gh pr view <N> --json state` ritorni `MERGED`, poi cleanup worktree + branch locale. Gating: `## LGTM` → auto-merge; `🔴 Important` o 🔴 process "missing sections" → no auto-merge, agent legge la review e applica fix in nuovo commit sullo stesso branch (re-review automatica). Altre CI (test/lighthouse/build) NON attese — osserva su `main` post-merge. PR che modificano `.github/workflows/pr-review-loop.yml` auto-mergiano via **drift-fallback deterministico** in `scripts/ci/auto-merge-eval.mjs` (autore fidato + `pr-body-contract` verde; vitest+collision restano gate). Dettaglio: `docs/AGENTS-HISTORY.md#lgtm-automerge`.
- **Attesa PR = watch ATTIVO nel turno, MAI stop idle.** Merge e review sono eventi GitHub Actions/Cloudflare → **l'harness NON li traccia, non ti re-invoca da solo**. Dopo open/update di una PR **non chiudere il turno finché non è risolta** — poll BREVE ricorrente (`gh pr view <N> --json state,reviews` ogni ~30-60s) fino a stato terminale (`MERGED` / `## LGTM` / `🔴 Important`), **posta status a ogni cambio**. Per stato esterno non tracciabile usa `ScheduleWakeup` (~270s per restare nella cache TTL 5min, o 1200s+ se il segnale cambia lento), MAI 300s netti. Sequenza: review → `## LGTM`/🔴 → auto-merge → `MERGED` → cleanup → verifica live. Dettaglio: `docs/AGENTS-HISTORY.md#active-pr-watch`.
- **Workflow-validation drift (reviewer non parte, anche se la PR NON tocca i workflow):** la GitHub App del reviewer esige che `.github/workflows/pr-review-loop.yml` (e gli altri workflow di review) sul branch PR siano **byte-identici** alla versione su `main`. Se `main` li aggiorna dopo che hai branchato → job `review` fallisce in <2min con `401 Unauthorized — Workflow validation failed`, nessun `## LGTM`. Non è un problema di auth, e re-run è inutile. **Fix:** `git fetch origin main && git merge origin/main` nel branch, poi push. Profilassi: fai `merge origin/main` prima di aspettarti il reviewer. Dettaglio: `docs/AGENTS-HISTORY.md#workflow-validation-drift`.
...
- **Leak locale di worktree/branch.** Il cleanup ancorato all'evento-merge nel turn dell'agent NON copre 3 buchi: (a) **EnterWorktree** auto-rimuove la *dir* worktree se unchanged ma **lascia il branch `worktree-agent-<id>`** orfano (0-ahead); (b) **squash-merge**: GitHub auto-cancella il remoto ma il branch **locale** resta e `git branch --merged` lo vede *unmerged* → mai potato; (c) **sessione morta/timeout**: l'agent non raggiunge mai il pre-task-close. Mitigazione: il pre-task-close audit DEVE girare **`node scripts/prune-merged-worktrees.mjs`** (dry-run) e applicare con `--apply`. Lo script: rimuove worktree il cui branch ha PR `MERGED|CLOSED` o è detached/abbandonato; cancella i branch locali `worktree-agent-*` 0-ahead e i locali la cui PR è `MERGED|CLOSED`; **report-only** (mai auto-delete) per worktree clean con commit ahead e senza PR (lavoro pre-PR potenzialmente vivo). Il **case (c) è chiuso da DUE hook** (`.claude/settings.json`): (1) **SessionEnd** gira `prune-merged-worktrees.mjs --apply --orphans-only` (istantaneo, safe non-presidiato); (2) **SessionStart** gira il **full** `--apply` in **background** → cattura i leftover delle sessioni morte. **Consultazione ISSUE-state**: un branch `fix/issue-N` **senza PR** ma con **issue #N CLOSED** viene cancellato anche se ORFANO. I worktree **Codex** (fuori dagli hook Claude) e i branch con remoto cancellato restano **report-only** con flag `upstream-GONE`. NB: lo script tocca SOLO worktree in `.claude/worktrees`/`.worktrees` — un worktree in path non-canonico NON viene potato: non crearne fuori dalle dir canoniche. Le anomalie **remote** le spazza il cron zero-Claude `worktree-branch-janitor.yml` (gh-API, age-gate `STALE_DAYS=60`, trigger `pull_request:[closed]` per cancellazione immediata dei close-non-merged). Dettaglio/incidenti: `docs/AGENTS-HISTORY.md#worktree-branch-leak`.
- **`git push` che timeouta/POSTa un pack enorme per un diff minimo** = repo locale senza manutenzione (molti pack disgiunti, delta cross-pack non chiusi) — non un problema di rete. Sintomo: `git rev-list --objects origin/main..HEAD` riporta pochi oggetti ma il push reale trasferisce centinaia di MB (`RPC failed; curl 28 Operation too slow` / `unexpected disconnect`). Diagnosi: `git count-objects -vH` (molti `packs`, `prune-packable` alto) + `git multi-pack-index write --bitmap` che fallisce con `Packfile doesn't have full closure` conferma delta cross-pack. Fix: `git repack -a -d -b` (consolida in un pack + bitmap; richiede headroom disco ~2× la dimensione pack corrente) poi `git maintenance start` (schedula gc/repack incrementale, previene la ri-bloat). L'object store è condiviso da TUTTI i worktree (`git worktree add` punta allo stesso `.git`), quindi il fix vale per l'intero clone, non solo per il branch in corso. Dettaglio/incidente: `docs/AGENTS-HISTORY.md#git-repo-maintenance`.
...
- **Review gateata su test-verdi → porta la suite a verde PRIMA di `gh pr create`.** `pr-review-loop` ora triggera su `workflow_run[tests]==success` (NON più su `pull_request`): la review Claude non parte MAI su una PR coi test rossi → aprire una PR rossa = ciclo sprecato (la review arriva solo dopo fix→re-test verde; e auto-merge resta comunque bloccato dal gate vitest). Regola agent locale: **gira i test localmente e portali verdi prima di aprire la PR** — per un fix mirato `npx vitest run <area toccata>`; per la suite intera serve `data/jobs.json` assemblato + `onnxruntime-node` (`node scripts/assemble-jobs-dataset.mjs && npm test`). Mai aprire PR con vitest rosso. Razionale: elimina il pattern «PR → review OK → test rossi → re-push → review di nuovo» che bruciava 2× quota Max sul burner #1 del loop (la review è serializzata DOPO i test, quindi la latenza happy-path la paghi solo se i test passano — se sono rossi non spendi review affatto).
