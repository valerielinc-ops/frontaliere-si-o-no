...
- **Auth Codex Luna Max = SOLO `CODEX_AUTH_JSON`** (subscription, zero costo API) per i workflow agentici `tests`, `issue-fix` e `post-merge-followup`; il wrapper locale `claude-codex-fallback` inoltra la sessione al broker Codex, ma questi percorsi usano Codex Luna Max e non un provider Claude/Anthropic di fallback. **Mai aggiungere `ANTHROPIC_API_KEY` o `CLAUDE_CODE_OAUTH_TOKEN`** ai workflow agentici.
  - **Eccezione scoped (owner-approved 2026-07-28, issue #4495; Haiku → Codex il 2026-09-24)**: `functions/src/codexFallback.js` è l'ultimo rung, a pagamento, delle fallback chain di `geminiGenerate.js`/`chatbotInference.js` — DOPO Gemini e tutti gli OpenAI-compatible free (Groq/NVIDIA). Chiama l'API OpenAI (Chat Completions, `reasoning_effort: none`) con `OPENAI_API_KEY` e `CODEX_FALLBACK_MODEL` (default `gpt-5.6-luna`, lo stesso id Codex della CI) da Remote Config; senza chiave il rung si salta (`codex: not_configured`). Sono Cloud Functions stateless che servono utenti, non workflow agentici: qui `CODEX_AUTH_JSON` (subscription del proprietario, refresh token monouso condivisi con la CI) non si usa, e `OPENAI_API_KEY` resta fuori da `RC_TO_ENV` perché non arrivi ai job CI. Il valore reale della chiave si provisiona direttamente in Remote Config (nessun template Remote Config è checked-in nel repo).
