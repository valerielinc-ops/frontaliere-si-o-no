...
- PR ready per `main` → reviewer Claude posta review e `auto-merge-on-lgtm.yml` mergia solo con la stringa esatta `## LGTM`. **NIENTE merge manuale, mai:** aspetta `MERGED`, poi cleanup worktree + branch locale. `🔴 Important` o 🔴 process "missing sections" blocca l'auto-merge: applica il fix nello stesso branch e lascia partire la re-review. Le altre CI si osservano su `main` post-merge; il drift-fallback di `scripts/ci/auto-merge-eval.mjs` copre le PR su `pr-review-loop.yml`.
- **Attesa PR = watch ATTIVO nel turno, MAI stop idle.** Dopo open/update, poll `gh pr view <N> --json state,reviews` ogni ~30-60s fino a `MERGED` / `## LGTM` / `🔴 Important`, con status a ogni cambio; se lo stato esterno non è tracciabile usa `ScheduleWakeup` (~270s o 1200s+), mai 300s netti. Dal 2026-08-24 `pr-watch-register.mjs` registra la PR e `pr-watch-gate.mjs` blocca finché la propria è `MERGED`/`CLOSED` o la review `claude[bot]` sull'ultimo commit contiene `## LGTM`. Fail-safe solo su errori d'infrastruttura; `sessionId` filtra proprie/legacy; nei run `GITHUB_ACTIONS=true` si sfila. Dettaglio: `docs/AGENTS-HISTORY.md#active-pr-watch`.
- **Il polling della PR include i CONFLITTI, e l'oracolo non è `mergeable`.** `mergeable` è asincrono e può restare `UNKNOWN`; misura con `git fetch origin main && git merge-tree --write-tree origin/main HEAD` (exit 1 = conflitto, righe `<mode> <oid> <stage>\t<path>` = file). `pr-autorebase.mjs` applica label/commento e li rimuove quando il conflitto rientra.
- **Workflow-validation drift:** i workflow di review devono essere byte-identici a `main`; se `main` avanza dopo il branch, `review` può fallire con `401 Unauthorized — Workflow validation failed` senza `## LGTM`. Fai `git fetch origin main && git merge origin/main` e pusha. Dettaglio: `docs/AGENTS-HISTORY.md#workflow-validation-drift`.
