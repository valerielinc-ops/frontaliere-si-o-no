...
- **Auth Codex Luna Max = SOLO `CODEX_AUTH_JSON`** (subscription, zero costo API) per i workflow agentici `tests`, `issue-fix` e `post-merge-followup`; l'action conserva la credenziale nel broker privato e passa ai consumer solo il socket. **Mai aggiungere `ANTHROPIC_API_KEY` o `CLAUDE_CODE_OAUTH_TOKEN`** ai workflow: i riferimenti ai vecchi nomi/action sono compatibilità, non un provider attivo.
...
- Leve frugalità attive:
  - **`issue-triage` = ZERO agente** (classificazione regex in bash) → eliminati ~50 run/giorno, il driver principale del session-limit.
...
- Pipeline: monitor → issue → `issue-triage` (classify+route **deterministico, zero agente**; applica `agent:fix` via `GITHUB_PAT` da RC) → `issue-fix` (Codex Luna Max: fix→PR) → `pr-review-loop` (Codex Luna Max, `## LGTM`) → `auto-merge-on-lgtm` (merge via PAT) → deploy → `post-merge-followup` (Codex Luna Max, batch 1 issue/PR). Contratto completo in `ISSUES.md` / `FOLLOWUP.md`.
