...
const RAW = {
  ...
  fr: ['Friburgo', 'FR', 'https://appls.ocn.ch/ecari-auction/ui/app/init?locale=fr_ch', { ...ACTIVE_ECARI, termsOfUse: 'official OCN auction portal; review source terms before increasing frequency', status: 'blocked', notes: 'Blocked: OCN still advertises this eCari shop (https://www.ocn.ch/fr/conduire/plaques-et-carte-grise/vente-encheres-de-plaques embeds https://www.ocn.ch/online-services/eauction-fr.html, which links only appls.ocn.ch), but the host does not answer. Re-measured 2026-09-25: appls.ocn.ch resolves to 194.209.14.250, yet port 443 returns no TLS bytes and closes after about 12 s, and port 80 ends in an upstream connect timeout (HTTP 503 from the egress gateway, which did try the host, so this is not a policy block). First seen 2026-09-18 from two networks (CI run 35091149727: Timeout fetching). No alternative OCN host exists. Next check 2026-10-02, or sooner if OCN changes the link; re-activate only once the eCari tabs are served and parsed again.' }],
  ...
  ti: ['Ticino', 'TI', 'https://www.carieauktion.ti.ch/ecari-auktion/', { ...ACTIVE_ECARI, termsOfUse: 'official Ticino eCari catalogue; review source terms before increasing frequency', status: 'blocked', notes: 'Blocked: https://www4.ti.ch/di/sc/veicoli/asta-targhe still links this URL, but re-measured 2026-09-25 with a browser User-Agent every path of www.carieauktion.ti.ch (/, /ecari-auktion/, /ecari-auktion/ui/app/init, /ecari-auction/, over http and https) answers HTTP 200 with the 6,967-byte cantonal "Pagina non disponibile" page and an F5 Support ID, so no catalogue markup is served: not a UA block and not a parser regression (first seen 2026-09-18). carieauktion.ti.ch without www has no DNS record, and www.aste.ti.ch is the debt-enforcement portal, not plates, so no alternative endpoint exists. Next check 2026-10-02; re-activate only when the eCari tabs are served again.' }],
