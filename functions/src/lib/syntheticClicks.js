/**
 * syntheticClicks.js — the single definition of "this click is not a person".
 *
 ...
 *
 * WHY IT LIVES UNDER functions/. Reviewing PR #5774 turned up a THIRD site of
 * the same defect — `calculateEngagementScore` in ./engagementScore.js, which
 * the five ESP webhook handlers call after every counter increment. That one is
 * a Cloud Function, and functions/src/ is a hermetic deploy package with zero
 * relative imports that escape it, so nothing outside functions/ can be
 * imported by it. The traffic already runs the other way (services/ and
 * scripts/ import functions/src/lib/*.js in eight places today), which makes
 * this directory the only home all three runtimes can reach.
 * scripts/lib/syntheticClicks.mjs is now a re-export shim onto this file.
 *
 * The three callers, and what each can see:
 ...
 *   - engagement score — ./engagementScore.js, on `newsletter_subscribers`:
 *     one click, from the document's own memory (`last_click_at` +
 *     `last_clicked_url`), so only the URL rules can fire. That is enough for
 *     the rule that matters: a click on the way out must not buy engagement.
 *
 * PURE. No I/O, no `Date.now()`.
 */

import { EMAIL_SCANNER_IP_RANGES } from './emailScannerRanges.js';
