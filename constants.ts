...
export const calculateDynamicExpenses = (familyMembers: number, target: 'CH' | 'IT') => {
  ...
  if (target === 'CH') {
    return [
      ...
      { label: 'SIM Mobile', amount: familyMembers === 1 ? 30 : Math.round(30 * Math.min(familyMembers, 3) * 0.7), frequency: 'MONTHLY', icon: 'Smartphone', tooltip: 'Solo abbonamento voce/dati (no rate telefono).' },
...
export const PRESET_EXPENSES_CH = [
  ...
  { label: 'Internet Casa', amount: 60, frequency: 'MONTHLY', icon: 'Wifi' },
  { label: 'Canone TV (Billag/Serafe)', amount: 335, frequency: 'ANNUAL', icon: 'Tv', tooltip: 'Canone TV obbligatorio (Billag/Serafe): 335 CHF/anno fisso.' },
  { label: 'SIM Mobile', amount: 30, frequency: 'MONTHLY', icon: 'Smartphone', tooltip: 'Solo abbonamento voce/dati (no rate telefono).' },
