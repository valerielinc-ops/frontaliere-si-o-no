      ...
      <div className="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 space-y-4">
        ...
        <div className="space-y-3">
          ...
          {linkedInAvailable && (
            <button
              type="button"
              onClick={() => { if (selectedJob) { saveAuthJobContext({ slug: selectedJob.slug, company: selectedJob.company, location: selectedJob.location, category: selectedJob.category }); const jobSlug = selectedJob.slugByLocale?.[locale] ?? selectedJob.slug; const section = getJobBoardSectionSlug(locale); const prefix = locale === 'it' ? '' : `/${locale}`; signInWithLinkedIn(`${prefix}/${section}/${jobSlug}/`.replace(/\/+/g, '/')); } else { signInWithLinkedIn(); } }}
