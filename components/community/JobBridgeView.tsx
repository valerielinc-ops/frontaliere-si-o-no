...
export default function JobBridgeView({ targetSlug, jobData, relatedJobs = [], onBack }: JobBridgeViewProps) {
  ...
  return (
    <div className="space-y-5 max-w-2xl mx-auto">
      ...
      {/* Blue banner */}
      <div className="rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 px-4 py-3 text-sm text-blue-800 dark:text-blue-300">
        {BANNER_COPY[locale] ?? BANNER_COPY.it}
      </div>

      {/* Countdown + direct link */}
      <div className="flex flex-col items-start gap-2">
        <p className="text-sm text-slate-500 dark:text-slate-400">{redirectCopy}</p>
        <a
          href={targetPath}
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/60 p-5 text-center space-y-3">
        ...
        {linkedInAvailable && (
          <button
            type="button"
            onClick={() => { saveAuthJobContext({ slug: targetSlug, company: jobData?.company, location: jobData?.location }); signInWithLinkedIn(listingPath); }}
