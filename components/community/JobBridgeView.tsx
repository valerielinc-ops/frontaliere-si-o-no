...
export default function JobBridgeView({ targetSlug, jobData, relatedJobs = [], onBack, hasAccess: hasAccessProp }: JobBridgeViewProps) {
 ...
 {localizedTitle && (
 <div>
 <h1 className="text-xl font-bold font-display text-strong leading-snug break-words [hyphens:auto]">{localizedTitle}</h1>
 ...
 {jobData?.location && (() => {
 const locationSlug = `${LOCATION_ROUTE_PREFIX[locale] || 'localita'}-${slugifyLocationName(jobData.location)}`;
 const locationHref = `${prefix}/${sectionSlug}/${locationSlug}/`.replace(/\/+/g, '/');
 return (
 <a href={locationHref} className="flex items-center gap-1 hover:text-accent hover:underline underline-offset-2 transition-colors">
 <MapPin size={14} />
 {jobData.location}
 </a>
 );
