...
function NewJobsCounter({ newJobsCount, matchingCount, onDismiss }: NewJobsCounterProps) {
  ...
  return (
    <div
      role="status"
      className={`flex items-start sm:items-center gap-2 px-3 py-2 rounded-[6px] bg-accent-subtle border border-accent-border/40 text-sm text-body transition-all duration-200 motion-reduce:transition-none ${dismissed ? 'h-0 overflow-hidden opacity-0 py-0 border-0' : ''}`}
    >
