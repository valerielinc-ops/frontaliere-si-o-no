...
const CommunityForum: React.FC = () => {
 ...
 reportCaughtError(e, 'forum.loadAnswers');
 } finally {
 ...
 authorPhoto: getUserPhotoURL(user, user?.uid),
 ...
 reportCaughtError(e, 'forum.createQuestion');
 } finally {
 setSubmitting(false);
 }
 ...
 reportCaughtError(e, 'forum.createAnswer');
 } finally {
 ...
 reportCaughtError(e, 'forum.upvoteQuestion');
 }
 }, [user]);
 ...
 reportCaughtError(e, 'forum.upvoteAnswer');
 }
 }, [user]);
 ...
 <span key={tag} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 bg-surface-raised text-muted rounded text-xs">
 ...
 <img src={selectedQuestion.authorPhoto} alt="Author avatar" className="w-5 h-5 rounded-full" referrerPolicy="no-referrer" width={20} height={20} loading="lazy" />
 ) : (
 ...
 {answer.authorPhoto ? (
 <img src={answer.authorPhoto} alt="Author avatar" className="w-4 h-4 rounded-full" referrerPolicy="no-referrer" width={16} height={16} loading="lazy" />
 ) : (
 ...
 spellCheck={true}
 className="w-full px-3 py-2 rounded-lg bg-surface-alt border border-edge text-sm min-h-[80px] resize-y"
 />
 ...
 className="flex-1 px-3 py-2 rounded-lg bg-surface border border-edge text-sm font-bold"
 >
 ...
 {/* Sort */}
 <div className="flex gap-1 bg-surface rounded-lg border border-edge p-0.5">
 <button
 onClick={() => setSortBy('recent')}
 className={`flex items-center gap-1 px-3 py-1.5 rounded-md text-xs font-bold transition-colors ${
 sortBy === 'recent' ? 'bg-accent-subtle text-accent' : 'text-muted'
 }`}
 >
 ...
 className={`flex items-center gap-1 px-3 py-1.5 rounded-md text-xs font-bold transition-colors ${
 sortBy === 'popular' ? 'bg-accent-subtle text-accent' : 'text-muted'
 }`}
 >
 ...
 className="w-full px-4 py-2.5 rounded-lg bg-surface-alt border border-edge text-sm font-bold"
 maxLength={200}
 />
 <textarea
 value={newBody}
 ...
 spellCheck={true}
 className="w-full px-4 py-2.5 rounded-lg bg-surface-alt border border-edge text-sm min-h-[120px] resize-y"
 />
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
 <div>
 <label className="block text-sm font-bold text-muted mb-1">{t('forum.category') || 'Categoria'}</label>
 <select
 value={newCategory}
 onChange={(e) => setNewCategory(e.target.value as ForumCategory)}
 aria-label={t('forum.category') || 'Categoria'}
 className="w-full px-3 py-2 rounded-lg bg-surface-alt border border-edge text-sm"
 >
 {FORUM_CATEGORIES.map(cat => (
 ...
 <div>
 <label className="block text-sm font-bold text-muted mb-1">{t('forum.tags') || 'Tag (separati da virgola)'}</label>
 <input
 type="text"
 value={newTags}
 ...
 {/* Content */}
 <div className="flex-1 min-w-0">
 <h3 className="font-bold text-sm text-strong line-clamp-2">
 {q.title}
 </h3>
 ...
 <span key={tag} className="px-1.5 py-0.5 bg-surface-raised text-muted rounded text-xs">
 {tag}
 </span>
 ))}
