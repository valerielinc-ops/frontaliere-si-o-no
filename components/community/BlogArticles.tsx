...
function BlogArticles({
 ...
 const makeInlineAd = (keyPrefix: string): ReactElement | null => {
  ...
  return (
   <Suspense
    key={`${keyPrefix}-ad-${n}`}
    fallback={<div style={{ ['--ad-mh']: `${slotConfig.placeholderMinHeight}px` } as CSSProperties} className="my-4 min-h-[var(--ad-mh)] [contain:content]" />}
   >
    <AdSenseBanner
 ...
 style={{ ['--sx']: `scaleX(${readingProgress / 100})` } as CSSProperties}
 role="progressbar"
 ...
 {idx === 2 && (
 <Suspense fallback={<div style={{ ['--ad-mh']: `${AD_SLOTS.JOBLIST_END_MULTIPLEX.placeholderMinHeight}px` } as CSSProperties} className="sm:col-span-2 lg:col-span-3 min-h-[var(--ad-mh)] xl:min-h-[600px] [contain:content]" />}>
 <AdSenseBanner
