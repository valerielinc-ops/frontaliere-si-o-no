...
const WeeklyDigest: React.FC = () => {
 ...
 if (needsConfirmation) {
   setStatus('pending');
   Analytics.trackUIInteraction('weekly_digest', 'subscribe', 'confirmation_pending', email.split('@')[1]);
   return;
 }
 markNewsletterSubscribedLocally();
 ...
 useEffect(() => {
   ...
   void (async () => {
     ...
     try {
       ...
       if (!database) throw new Error('Firestore non disponibile');
       await upsertNewsletterSubscriber(database, {
         email: socialEmail,
         userId: user.uid,
         source: 'weekly_digest_social_consent',
         sourceChannel: 'weekly_digest',
         sourcePage: typeof window !== 'undefined' ? window.location.pathname : '/comunicazioni/',
         sourceCta: 'weekly_digest_social_signup',
         sourceComponent: 'WeeklyDigest',
 ...
 }, [locale, pendingSocialMethod, user]);
 ...
 if (status === 'pending') {
 return (
 <div className="max-w-2xl mx-auto bg-info-subtle border border-info-border rounded-2xl p-4 sm:p-6 text-center" role="status" aria-live="polite">
 <Mail size={32} className="text-info mx-auto mb-3" />
 <h3 className="font-bold text-strong">{t('newsletter.doubleOptIn.title')}</h3>
 ...
 />
 {!googleButtonReady && (
 <button type="button" onClick={() => { setPendingSocialMethod('google_oauth'); void googleSignIn(); }} className="w-full min-h-[40px] grid grid-cols-[20px_1fr_20px] items-center px-4 py-2 bg-surface border border-edge rounded-xl text-body text-xs font-semibold hover:bg-surface-raised transition-colors">
 ...
 {linkedInAvailable && (
 <button type="button" onClick={() => { setPendingSocialMethod('linkedin_oauth'); void signInWithLinkedIn(); }} className="flex-1 min-h-[44px] inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-brand-linkedin hover:bg-brand-linkedin-hover text-on-accent text-sm font-semibold transition-colors">
