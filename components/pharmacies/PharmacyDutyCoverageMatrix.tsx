...
import { pharmacyById, pharmacyCitySlug } from '@/services/pharmacies/data';
...
export default function PharmacyDutyCoverageMatrix({
  ...
  const matrix = useMemo(() => buildDutyCoverageMatrix({ locale, now, weekStart, duties, catalogue, registry }), [catalogue, duties, locale, now, registry, weekStart]);
  const copy = getDutyCoverageMatrixCopy(locale);

  return <section className="space-y-6" aria-labelledby="pharmacy-duty-coverage-matrix-heading" data-coverage-matrix="true" data-release-ready={String(matrix.releaseReady)}>
