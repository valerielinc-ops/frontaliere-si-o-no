...
const CurrencyExchange: React.FC = () => {
 ...
 className="w-full pl-20 pr-4 py-3 bg-surface-raised border border-edge rounded-xl font-bold text-subtle text-lg"
 />
 </div>
 ...
 <span className="text-xs font-semibold text-success uppercase">{t('currency.best_offer')}</span>
 </div>
 <div className="text-sm font-semibold text-strong mb-1">{best.provider.name}</div>
 <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 text-sm">
 <span className="font-semibold text-success">€ {best.netAmount.toFixed(2)}</span>
 ...
 <div className="bg-danger-subtle rounded-xl sm:rounded-2xl border border-danger-border p-3 sm:p-5">
 <div className="flex items-center gap-2 mb-2">
 <TrendingDown className="text-danger" size={16} />
 <span className="text-xs font-semibold text-danger uppercase">{t('currency.worst_offer')}</span>
 </div>
 <div className="text-sm font-semibold text-strong mb-1">{worst.provider.name}</div>
 <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 text-sm">
 <span className="font-semibold text-danger">€ {worst.netAmount.toFixed(2)}</span>
 ...
 <div className="text-xl sm:text-2xl font-bold text-strong">
 ...
 <div className="text-sm sm:text-lg font-bold text-strong">
 ...
 <span
 key={fidx}
 className="px-2 sm:px-2.5 py-0.5 sm:py-1 bg-surface-raised text-body text-xs font-medium rounded-md sm:rounded-lg"
 >
 ...
 <Suspense fallback={<div style={{ ['--ph-min-h']: AD_SLOTS.ARTICLE_INLINE_MOBILE.placeholderMinHeight } as React.CSSProperties} className="my-6 min-h-[var(--ph-min-h)] [contain:content]" />}><AdSenseBanner adSlot={AD_SLOTS.ARTICLE_INLINE_MOBILE.slot} adFormat={AD_SLOTS.ARTICLE_INLINE_MOBILE.format} adLayout={AD_SLOTS.ARTICLE_INLINE_MOBILE.layout} fullWidthResponsive={false} className="my-6" /></Suspense>
