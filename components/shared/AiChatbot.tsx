 ...
 * — tries gemini-flash-lite-latest → gemini-flash-latest internally
 * 2. Browser-side direct Gemini call (fallback when Function is unreachable)
 * — uses gemini-flash-lite-latest (alias sullo stable corrente)
...
const AiChatbot: React.FC<AiChatbotProps> = ({ isLoggedIn, onSignIn, onSignInFacebook, onContinueWithEmail, hideOnMobile }) => {
 ...
 eagerAuth();
 // Authentication from the assistant grants chat access only; it does not
 // create or reactivate newsletter state. The screen is declared in
 ...
 size: 'large',
 locale,
 attribution: AI_CHATBOT_AUTH_ATTRIBUTION,
 ...
 <button
 type="button"
 onClick={() => signInWithLinkedIn(undefined, AI_CHATBOT_AUTH_ATTRIBUTION)}
