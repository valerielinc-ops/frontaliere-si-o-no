      ...
      {isOpen && queueActive && (
        <div className={`fixed right-4 z-[53] w-[360px] max-w-[calc(100vw-2rem)] h-[500px] max-h-[calc(100vh-6rem)] bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden animate-fade-in ${hasBottomOverlay ? 'bottom-48 md:bottom-4' : 'bottom-24 md:bottom-4'}`}>
          ...
          <div className="px-3 py-3 border-t border-slate-200 dark:border-slate-700 shrink-0">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-base md:text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-700"
                style={{ fontSize: '16px' }}
                disabled={loading}
          ...
          {authGateOpen && !canChat && (
            <div className="absolute left-0 right-0 top-[56px] bottom-0 z-20 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm p-4 overflow-y-auto">
