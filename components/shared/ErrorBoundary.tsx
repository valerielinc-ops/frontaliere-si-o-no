...
export class SilentErrorBoundary extends Component<SilentBoundaryProps, SilentBoundaryState> {
 ...
 //
 // isModuleParseError included (#5531): a stale SafeLazy widget chunk can
 // resolve as SPA-fallback HTML parsed as a module ("Unexpected identifier
 // '<word>'") the same way a primary lazyRetry()-wrapped chunk can — see
 // resilientImport.ts. Unlike the isVersionSkewError widening the top-level
 // ErrorBoundary/ChunkLoadErrorBoundary deliberately don't take (by
 // construction, PR #5535: those paths RELOAD the page on a match, so a
