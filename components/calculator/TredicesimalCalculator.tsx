...
const TredicesimalCalculator: React.FC = () => {
  ...
  const formatCurrency = useCallback((amount: number, cur: string) => {
    return `${cur} ${amount.toLocaleString('it-CH', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;
  }, []);
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl mb-6 text-sm transition-colors"
        aria-label={t('tredicesima.whatIs')}
      >
        <span className="flex items-center gap-2 font-medium text-amber-700 dark:text-amber-300">
          <HelpCircle size={16} />
          {t('tredicesima.whatIs')}
        </span>
        {showInfo ? <ChevronUp size={16} className="text-amber-500" /> : <ChevronDown size={16} className="text-amber-500" />}
      </button>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 sm:p-6 mb-6 space-y-5">
        ...
        <div>
          <label htmlFor="tred-contract" className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">
            ...
            {t('tredicesima.contractType')}
          </label>
          <select
            id="tred-contract"
            value={contractType}
            onChange={e => { setContractType(e.target.value as ContractType); setCalculated(false); }}
          ...
          >
            <option value="swiss">{t('tredicesima.contract.swiss')}</option>
            <option value="italian_ccnl">{t('tredicesima.contract.italianCcnl')}</option>
            <option value="italian_other">{t('tredicesima.contract.italianOther')}</option>
          </select>
        </div>

        {/* Annual Gross */}
        <div>
          <label htmlFor="tred-gross" className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">
            <Euro size={14} className="inline mr-1" />
            {t('tredicesima.annualGross')} ({contractType === 'swiss' ? 'CHF' : 'EUR'})
          </label>
          <input
            id="tred-gross"
            type="number"
        ...
        {/* Payment Frequency */}
        <div>
          <label htmlFor="tred-freq" className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">
            <Calendar size={14} className="inline mr-1" />
            {t('tredicesima.paymentFreq')}
          </label>
          <select
            id="tred-freq"
            value={paymentFrequency}
            onChange={e => { setPaymentFrequency(e.target.value as PaymentFrequency); setCalculated(false); }}
            className="w-full px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 text-sm"
          >
            <option value="12">{t('tredicesima.freq.12')}</option>
            <option value="13">{t('tredicesima.freq.13')}</option>
            <option value="14">{t('tredicesima.freq.14')}</option>
          </select>
        </div>

        {/* Months Worked */}
        <div>
          <label htmlFor="tred-months" className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">
            <Calendar size={14} className="inline mr-1" />
            {t('tredicesima.monthsWorked')}
          </label>
          <input
            id="tred-months"
            type="range"
        ...
        {/* Quattordicesima toggle (Italian only) */}
        {contractType !== 'swiss' && (
          <div className="flex items-center justify-between">
            <label htmlFor="tred-quattordicesima" className="text-sm font-medium text-slate-700 dark:text-slate-200">
              <Award size={14} className="inline mr-1" />
              {t('tredicesima.hasQuattordicesima')}
            </label>
            <button
              id="tred-quattordicesima"
              role="switch"
              aria-checked={hasQuattordicesima}
              aria-label={t('tredicesima.hasQuattordicesima')}
              onClick={() => { setHasQuattordicesima(!hasQuattordicesima); setCalculated(false); }}
        ...
        {/* Exchange Rate (Swiss only) */}
        {contractType === 'swiss' && (
          <div>
            <label htmlFor="tred-rate" className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">
              <TrendingUp size={14} className="inline mr-1" />
              {t('tredicesima.exchangeRate')}
            </label>
            <input
              id="tred-rate"
              type="number"
        ...
        <button
          onClick={handleCalc}
          className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-amber-600 hover:bg-amber-700 text-white font-semibold rounded-xl transition-colors"
          aria-label={t('tredicesima.calculate')}
        >
          <Calculator size={18} />
          {t('tredicesima.calculate')}
        </button>
      </div>
        ...
        <>
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 sm:p-6 mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* Quattordicesima (if applicable) */}
              {hasQuattordicesima && contractType !== 'swiss' && (
                <div className="bg-orange-50 dark:bg-orange-900/20 rounded-xl p-4 text-center">
                  <p className="text-xs text-orange-700 dark:text-orange-300 mb-1">{t('tredicesima.results.quattordicesima')}</p>
                  <p className="text-lg font-bold text-orange-700 dark:text-orange-300">{formatCurrency(result.quattordicesima, result.currency)}</p>
                </div>
              )}
          ...
          <Suspense fallback={null}>
            <LeadMagnetCTA variant="salary_guide" delay={3000} />
          </Suspense>

          {/* Shareable result card */}
          <Suspense fallback={null}>
            <ShareableResultCard
              title={t('tredicesima.title') || 'Calcolo Tredicesima'}
              subtitle={`${inputs.annualGross.toLocaleString('it-CH')} ${result.currency}/anno`}
              rows={[
