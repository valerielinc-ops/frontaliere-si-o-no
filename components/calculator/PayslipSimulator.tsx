...
const PayslipSimulator: React.FC<PayslipProps> = ({ userProfile }) => {
 ...
 {LPP_RATES.map(r => (
 <div key={r.minAge} className={`flex items-center justify-between py-1 px-2 rounded-lg ${age >= r.minAge && age <= r.maxAge ? 'font-bold text-warning bg-warning-subtle' : ''}`}>
 <span>{r.minAge}-{r.maxAge} {t('payslip.age').toLowerCase()}</span>
 <span>{(r.rate * 100).toFixed(1)}%</span>
 </div>
 ))}
 <p className="mt-2 text-xs text-muted">{t('payslip.anniFiglio')}</p>
 </div>
 )}
 </div>
 );
