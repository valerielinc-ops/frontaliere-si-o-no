  ...
  return (
    <div className="space-y-4 pb-3">
      ...
      {result && !showFullResults && !sheetOpen && (
        <button
          onClick={openSheet}
          className="w-full flex items-center justify-center gap-2 py-2 text-[11px] font-semibold text-slate-500 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-400 transition-colors"
        >
          <Settings2 size={12} />
      ...
      {showNewsletterGate && (
        <>
          <div className="fixed inset-0 bg-black/50 z-[70] animate-fade-in" onClick={dismissGate} />
          <div className="fixed inset-x-4 top-1/2 -translate-y-1/2 z-[71] bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-sm mx-auto overflow-hidden animate-fade-in">
            ...
            <div className="p-5 space-y-3">
              ...
              <button
                onClick={dismissGate}
                className="w-full text-center text-xs text-slate-500 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 py-1"
              >
