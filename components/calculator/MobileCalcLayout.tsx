...
const MOBILE_CALC_AUTH_ATTRIBUTION = { cta: 'mobile_calc_gate_social', component: 'MobileCalcLayout' } as const;
...
const MobileCalcLayout: React.FC<Props> = ({
 ...
 attribution: MOBILE_CALC_AUTH_ATTRIBUTION,
 });
 ...
 <button type="button" onClick={() => googleSignIn(MOBILE_CALC_AUTH_ATTRIBUTION)} className="w-full min-h-[40px] grid grid-cols-[20px_1fr_20px] items-center px-4 py-2 bg-surface border border-edge rounded-xl text-body text-xs font-semibold hover:bg-surface-raised transition-colors">
 ...
 {linkedInAvailable && (
 <button type="button" onClick={() => signInWithLinkedIn(undefined, MOBILE_CALC_AUTH_ATTRIBUTION)} className="w-full min-h-[44px] inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-brand-linkedin hover:bg-brand-linkedin-hover text-on-accent text-sm font-semibold transition-colors">
