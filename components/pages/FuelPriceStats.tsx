...
function DetailSection({
  ...
  return (
    <div className="mt-4 space-y-4 rounded-[1.75rem] border border-edge bg-white/90 dark:bg-slate-800/90 p-4 sm:p-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-xl font-bold text-heading">{tt('fuelPrices.detailTitle', 'Dettaglio comune')}</h3>
        ...
        <div className="rounded-2xl border border-edge bg-slate-50/80 dark:bg-slate-900/50 p-4">
          ...
          <div className="mt-3 space-y-3">
            {row.italy.stations.slice(0, 12).map((station) => (
              <div key={`${station.id}-${station.priceEur}-${station.isSelf ? 'self' : 'served'}`} className="rounded-2xl border border-white dark:border-slate-700 bg-surface p-3">
        ...
        <div className="rounded-2xl border border-edge bg-slate-50/80 dark:bg-slate-900/50 p-4">
          ...
          <div className="mt-3 space-y-3">
            {row.swiss.nearbyStations.slice(0, 12).map((station) => (
              <div key={station.id} className="rounded-2xl border border-white dark:border-slate-700 bg-surface p-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-white dark:border-slate-700 bg-white/85 dark:bg-slate-800/85 px-4 py-3">
              <div className="text-xs font-semibold uppercase tracking-wide text-muted">{tt('fuelPrices.italySnapshot', 'Snapshot Italia')}</div>
              <div className="mt-1 font-bold text-heading">{formatDate(data.sources.italy.priceSnapshotDate, locale)}</div>
            </div>
            <div className="rounded-2xl border border-white dark:border-slate-700 bg-white/85 dark:bg-slate-800/85 px-4 py-3">
              <div className="text-xs font-semibold uppercase tracking-wide text-muted">{tt('fuelPrices.exchangeRate', 'Cambio CHF/EUR')}</div>
              <div className="mt-1 font-bold text-heading">1 CHF = {formatMoney(data.sources.exchangeRate.eurPerChf, 'EUR', locale, 4)}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="flex flex-wrap items-baseline gap-x-5 gap-y-1.5 text-sm text-subtle">
        ...
        <span className="inline-flex items-baseline gap-1.5"><span className="text-lg font-semibold text-link">{data.summary.cheaperSwissCount}</span> {tt('fuelPrices.cheaperSwissCount', 'Comuni dove conviene CH')}</span>
        <span className="hidden sm:inline text-slate-300 dark:text-slate-600" aria-hidden="true">·</span>
      ...
      <section className="rounded-[2rem] border border-edge bg-surface p-5 sm:p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-3 text-xl font-bold text-heading sm:text-2xl">{tt('fuelPrices.personalizedTitle', 'Dove ti conviene davvero fare benzina')}</h2>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <label className="sm:col-span-2 xl:col-span-2">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-edge bg-surface-alt/50 px-4 py-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:border-orange-500 dark:text-slate-200"
              />
            ...
            <div className="grid gap-3 lg:grid-cols-[1fr,auto,auto]">
              <label className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-2xl border border-edge bg-surface-alt/50 py-3 pl-10 pr-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:border-orange-500 dark:text-slate-200"
                />
              </label>
              <select
                value={province}
                ...
                className="rounded-2xl border border-edge bg-surface-alt/50 px-4 py-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:border-orange-500 dark:text-slate-200"
              >
                {provinces.map((item) => (
