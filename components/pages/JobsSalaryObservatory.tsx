...
function LeaderBlock(props: {
 ...
 <a
 key={`${item.key}-${item.url}`} href={leaderHref(item, props.locale)} className="flex items-center justify-between gap-3 rounded-2xl border border-edge px-3 py-2 no-underline transition-colors hover:border-accent" onClick={(event) => { event.preventDefault(); navigateTo({ activeTab: 'job-board', jobSlug: localizeLeaderSlug(item.url, props.locale) }, props.locale); }} > <div className="min-w-0"> <div className="text-xs font-bold font-display text-muted">#{index + 1}</div> <div className="line-clamp-2 text-sm font-semibold font-display text-heading">{item.name}</div> </div> <div className="shrink-0 text-right text-sm font-bold font-display text-accent"> {props.valueRenderer(item)} </div> </a> ))} </div> )} </div> ); } export const JobsSalaryObservatory: React.FC = () => { const { locale } = useTranslation(); const copy = COPY[locale] || COPY.it; const [data, setData] = useState<JobBoardStatsData | null>(null); const [loading, setLoading] = useState(true); const [refreshing, setRefreshing] = useState(false); const [error, setError] = useState<string | null>(null); const load = async (forceRefresh = false) => { try { setError(null); if (forceRefresh) setRefreshing(true); else setLoading(true); const result = await fetchJobBoardStats(forceRefresh); setData(result); } catch (err: any) { setError(err?.message || copy.empty); } finally { setLoading(false); setRefreshing(false); } }; useEffect(() => { Analytics.trackPageView(buildPath({ activeTab: 'stats', statsSubTab: 'jobs-observatory' }, locale), copy.title); /* Leader links may point at real job slugs: buildPath resolves their canton + localized slug via the job slug map (by-construction canton-correct links). The map is no longer idle-loaded on every route (issue #3526), so this corpus-wide consumer loads the monolith on demand, on this route only. */ void ensureJobSlugMapLoaded().catch(() => { /* non-critical: links degrade to legacy TI base, clicks still bridge */ }); void load(false); }, [locale, copy.title]); if (loading) { return ( <div className="rounded-3xl border border-edge bg-surface p-6 shadow-sm flex items-center justify-center"> <Loader2 className="h-5 w-5 animate-spin text-accent" /> </div> ); } if (!data) { return ( <div className="rounded-3xl border border-edge bg-surface p-6 text-sm text-subtle shadow-sm"> {error || copy.empty} </div> ); } const salary = data.salary?.coverage; const salaryLeaders = data.salary?.leaders; return ( <div className="space-y-6"> <div className="rounded-3xl border border-edge bg-surface p-6 shadow-sm"> <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"> <div className="max-w-4xl"> <h2 className="flex items-center gap-2 text-lg font-bold font-display text-heading"> <Wallet className="h-5 w-5 text-accent" /> {copy.title} </h2> <p className="mt-3 text-sm leading-7 text-subtle">{copy.intro}</p> <p className="mt-3 text-sm leading-7 text-muted">{copy.methodology}</p> <div className="mt-5 flex flex-wrap gap-2"> <ActionLink label={copy.allJobs} route={{ activeTab: 'job-board' }} locale={locale} /> <ActionLink label={copy.jobsToday} route={{ activeTab: 'job-board', jobSlug: locale === 'it' ? 'offerte-di-lavoro-ticino-oggi' : locale === 'en' ? 'ticino-jobs-today' : locale === 'de' ? 'jobs-tessin-heute' : 'offres-emploi-tessin-aujourdhui' }} locale={locale} /> <ActionLink label={copy.salaryCompare} route={{ activeTab: 'stats', statsSubTab: 'salary-compare' }} locale={locale} /> <ActionLink label={copy.overview} route={{ activeTab: 'stats', statsSubTab: 'overview' }} locale={locale} /> </div> </div> <div className="rounded-2xl border border-edge bg-surface-alt p-4"> <div className="text-xs font-bold font-display uppercase tracking-[0.16em] text-muted">{copy.updated}</div> <div className="mt-2 text-sm font-semibold font-display text-strong">{formatDate(data.generatedAt, locale)}</div> <button type="button" onClick={() => void load(true)} disabled={refreshing} className="mt-4 inline-flex items-center gap-2 rounded-xl border border-edge px-3 py-2 text-xs font-bold font-display text-body hover:border-accent" > <RefreshCw className={`h-4 w-4 ${refreshing ? 'animate-spin' : ''}`} />
 {copy.refresh}
 </button>
 </div>
 </div>
