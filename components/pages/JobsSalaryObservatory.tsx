  ...
  return (
    <div className="rounded-3xl border border-slate-100 bg-surface p-5 shadow-sm dark:border-slate-700">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <Kpi label={copy.kpiAverageMid} value={formatCurrency(salary?.avgMid || 0, locale)} accent="border-stripe-100 bg-stripe-50/60 dark:border-stripe-900/40 dark:bg-stripe-950/20" />
        ...
        <Kpi label={copy.kpiTrackedLocations} value={String(data.totals.activeLocations || 0)} accent="border-stripe-100 bg-stripe-50/60 dark:border-stripe-900/40 dark:bg-stripe-950/20" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <LeaderBlock title={copy.salaryCompanies} items={salaryLeaders?.topSalaryCompanies || []} locale={locale} valueRenderer={(item) => formatCurrency(Number((item as JobBoardSalaryLeader).weightedSalary || (item as JobBoardSalaryLeader).avgMid || 0), locale)} icon={<Building2 size={16} className="text-stripe-600" />} empty={copy.empty} />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <LeaderBlock title={copy.hottestTitles} items={data.leaders.topTitlesAdded30d || []} locale={locale} valueRenderer={(item) => String(Number(item.added || 0))} icon={<Search size={16} className="text-stripe-600" />} empty={copy.empty} />
        <LeaderBlock title={copy.hottestLocations} items={data.leaders.topLocationsAdded30d || []} locale={locale} valueRenderer={(item) => String(Number(item.added || 0))} icon={<MapPin size={16} className="text-stripe-600" />} empty={copy.empty} />
        <LeaderBlock title={copy.hiringCompanies} items={data.leaders.topCompaniesAdded30d || []} locale={locale} valueRenderer={(item) => String(Number(item.added || 0))} icon={<Briefcase size={16} className="text-stripe-600" />} empty={copy.empty} />
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <LeaderBlock title={copy.salaryCompanies} items={data.leaders.topCompaniesActive || []} locale={locale} valueRenderer={(item) => String(Number(item.count || 0))} icon={<TrendingUp size={16} className="text-stripe-600" />} empty={copy.empty} />
